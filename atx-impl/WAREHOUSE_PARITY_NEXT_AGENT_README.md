...
- S9d adds average-balance returns (`average_return_on_assets`, `average_return_on_equity`, `operating_cash_flow_to_average_assets` — denominator = mean of ending + prior-year balance, the Compustat return-on-capital convention) plus `equity_growth_yoy`, reusing the prior-year pairing. The derived-ratio surface now spans **32 ratios across 7 families** (profitability, leverage, efficiency, cash_flow, payout, per_share, growth) — comprehensive for the 15 canonical metrics currently extracted. Live rebuild grew `fundamental_ratios` to **7,728** rows / **32** ratio codes.
- Full DB test suite passed after S5k–S9d: `python -m pytest db\tests -q` (237 passed; +52 in `test_fundamental_ratios.py`).
- Live default DB is migrated through `0032`; live `fundamental_ratios=7,728`. Estimate counts after S5k smoke are `est_actual=1,240`, `est_surprise=1,222`, and `est_detail`/`est_consensus`/`est_guidance`/`est_recommendation`/`est_recommendation_summary`/`est_security_link=0` by design until licensed/manual files, accepted crosswalk evidence, or local SEC guidance text files are injected.
