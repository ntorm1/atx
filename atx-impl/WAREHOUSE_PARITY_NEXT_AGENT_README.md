...
0. **S6b — 13F value-unit cutover + curated continuity seed.** Apply the 2023-01-03 SEC 13F `VALUE`-unit cutover (×1000 for `periodOfReport <= 2022-12-31`) in `thirteenf.py` so pre-2023 backfills are not 1000× understated (no live impact today — all loaded periods are post-2023 — so this is a regression-tested correctness fix for future backfills). Then load a curated subadvisor / M&A-continuity seed CSV into `filer_13f_cik_alias` via `python scripts/build_filer_aliases.py --seed-file <csv>` (columns: `parent_cik,child_cik,alias_type,valid_from,confidence,evidence[,valid_to,available_at]`) and verify `resolve_primary_cik` rolls children up to parents PIT-correctly.
