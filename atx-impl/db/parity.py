...
PROVIDER_PARITY_ROWS: tuple[ProviderParityRow, ...] = (
    ...
    ProviderParityRow(
        ...
        factors_or_fields=("ATS dark-pool volume", "non-ATS member-firm volume", "trade count", "ATS market-share percent", "ATS venue count", "restatement flags"),
        open_substitute="FINRA OTC Transparency weekly/monthly/block off-exchange volume ingested (injectable CSV; production source is the FINRA Query API otcMarket group, OAuth2, tiered publication delay) into offexchange_volume + offexchange_venue, with a derived offexchange_security_period ATS-share rollup and a security-joined view.",
        warehouse_tables=("offexchange_venue", "offexchange_volume", "offexchange_security_period", "v_offexchange_security_period"),
        parity_status="partial",
        limitations="Schema, restatement handling (is_latest/restatement_seq), as-of APIs, quality checks, and lake exports are built and reproduce the FINRA blueprint fixture exactly, but the default DB is empty until a FINRA weeklySummary file is injected; the live OAuth2 FINRA Query API connector and a dedicated offexchange_quality_report table are not yet wired.",
