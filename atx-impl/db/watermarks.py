...
WATERMARK_QUERIES: tuple[str, ...] = (
    ...
    """,
    """
    SELECT 'finra_short_volume', 'max_trade_date', max(trade_date)::VARCHAR
    FROM finra_short_volume
    HAVING count(*) > 0
    """,
    """
    SELECT 'finra_short_volume', 'max_available_at', max(available_at)::VARCHAR
    FROM finra_short_volume
    HAVING count(*) > 0
    """,
    """
    SELECT 'short_volume_metrics', 'max_trade_date', max(trade_date)::VARCHAR
    FROM short_volume_metrics
    HAVING count(*) > 0
    """,
    """
    SELECT 'short_volume_metrics', 'max_available_at', max(available_at)::VARCHAR
    FROM short_volume_metrics
    HAVING count(*) > 0
    """,
    """
    SELECT 'offexchange_quality_report', 'max_period_start_date', max(period_start_date)::VARCHAR
    FROM offexchange_quality_report
    HAVING count(*) > 0
    """,
    """
    SELECT 'offexchange_quality_report', 'max_available_at', max(available_at)::VARCHAR
    FROM offexchange_quality_report
    HAVING count(*) > 0
    """,
    """
    ...
    """,
    """
    SELECT 'insider_transaction_metrics', 'max_signal_date', max(signal_date)::VARCHAR
    FROM insider_transaction_metrics
    HAVING count(*) > 0
    """,
    """
    SELECT 'insider_transaction_metrics', 'max_available_at', max(available_at)::VARCHAR
    FROM insider_transaction_metrics
    HAVING count(*) > 0
    """,
    """
    SELECT 'security_listing_metrics', 'max_as_of_date', max(as_of_date)::VARCHAR
    FROM security_listing_metrics
    HAVING count(*) > 0
    """,
    """
    SELECT 'security_listing_metrics', 'max_available_at', max(available_at)::VARCHAR
    FROM security_listing_metrics
    HAVING count(*) > 0
    """,
    """
