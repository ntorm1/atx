...
def _seed_catalog(store: DuckDBStore) -> None:
    ...
    dataset_rows = [
        ...
        ("est_consensus", "injected_estimates", "Consensus estimate snapshots", "Injectable consensus rows with mean/median/high/low/stdev and revision counts.", "security_id,measure,period_end,consensus_date", "est_consensus", "as_of_date", "available_at"),
        ...
        ("est_guidance", "injected_estimates", "Management guidance observations", "Injectable guidance rows, intended for SEC 8-K Item 2.02/7.01 extraction or licensed guidance feeds.", "security_id,measure,period_end,guidance_date", "est_guidance", "as_of_date", "available_at"),
        ("est_recommendation", "injected_estimates", "Broker recommendation observations", "Injectable recommendation/rating rows with broker and analyst identifiers.", "security_id,broker,analyst,rating_date", "est_recommendation", "as_of_date", "available_at"),
    ...
    table_rows = [
        ...
        ("est_consensus", "silver", "estimate_consensus", "security_id,measure_code,period_end,consensus_date", "Consensus estimate snapshots from injectable providers.", '["security_id","measure_code","period_end","consensus_date","source"]', "Use available_at and consensus_date/as_of_date for PIT visibility; consensus rows are append snapshots."),
        ...
        ("est_guidance", "silver", "estimate_guidance", "security_id,measure_code,period_end,guidance_date", "Management guidance observations from injectable providers.", '["security_id","measure_code","period_end","guidance_date","source"]', "Use available_at to model extraction/feed availability; public SEC 8-K NER is a follow-on source."),
        ("est_recommendation", "silver", "estimate_recommendation", "security_id,broker_id,analyst_id,rating_date", "Broker recommendation observations with canonicalized rating fields.", '["security_id","broker_id","analyst_id","rating_date","source"]', "Recommendation scales must be normalized to the chosen canonical direction before mixing providers."),
