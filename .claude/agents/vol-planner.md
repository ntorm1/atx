...
3. Per lane, produce a brief per `.agents/harness/TEMPLATES.md` "Lane brief": explicit files-in-scope, files-forbidden, and one `gate_closure` entry per scoped file. Start with every mandatory command from the workflow-owned exact path/path-pattern registry; additions may be additive but never substitute those commands. C/C++ entries name affected unit targets and anchored exact `-R` regexes; hypothesis work names OracleBench test IDs; headers name owning PCH-off targets. Labels, broad runners/builds, full regression suites, and full-repo hygiene are outside the oracle loop.
