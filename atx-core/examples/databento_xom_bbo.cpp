...
int main(int argc, char **argv) {
  ...
  try {
    ...
    std::printf("Query: %s %s  [%s, %s)  parent=%s\n\n", dataset.c_str(), ToString(kSchema),
                start.c_str(), end.c_str(), symbols.front().c_str());
    ...
    const auto cost = client.MetadataGetCost(dataset, range, symbols, kSchema, kStypeIn, 0);
    std::printf("Preflight (free): records=%llu  billable=%.3f MB  usage_cost=$%.6f\n",
                static_cast<unsigned long long>(records), static_cast<double>(bytes) / 1e6, cost);
    ...
    if (cost > cap) {
      std::printf("ABORT: estimated cost $%.6f exceeds cap $%.2f. No data pulled.\n"
    ...
    auto store = client.TimeseriesGetRangeToFile(dataset, range, symbols, kSchema, kStypeIn,
                                                 kStypeOut, 0, out);

    // ---- Replay, mapping instrument_id -> option symbol ----
    const auto &md = store.GetMetadata();
    // Build the symbol map on the requested UTC date. The original one-off XOM
    // tool hard-coded 2026-06-05, which made otherwise successful event-date
    // downloads report a spurious "Outside query range" error after the file was
    // already written.
    const int year = std::stoi(start.substr(0, 4));
    const int month = std::stoi(start.substr(5, 2));
    const int day_of_month = std::stoi(start.substr(8, 2));
    const date::year_month_day day = date::year{year} / month / day_of_month;
    PitSymbolMap syms = md.CreateSymbolMapForDate(day);

    std::printf("\n%-26s %8s %8s %8s  %8s x %-8s\n", "contract", "bid", "ask", "mid", "bidsz",
                "asksz");
    std::uint64_t rows = 0;
    for (const Record *rec = store.NextRecord(); rec != nullptr; rec = store.NextRecord()) {
      ...
      const std::string sym =
          it != syms.Map().end() ? it->second : std::to_string(q.hd.instrument_id);
      std::string mid = "      --";
      if (l.bid_px != kUndefPrice && l.ask_px != kUndefPrice) {
        char buf[32];
        std::snprintf(buf, sizeof(buf), "%8.2f",
                      (static_cast<double>(l.bid_px) + static_cast<double>(l.ask_px)) * 0.5 * 1e-9);
        mid = buf;
      }
      if (rows < 40) {
    ...
    std::printf("\nStored %llu BBO quotes across the %s chain.\nFile: %s (%llu bytes)\n",
                static_cast<unsigned long long>(rows), parent.c_str(), out.string().c_str(),
