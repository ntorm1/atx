...
| `hygiene` | PCH **OFF** — strict per-TU includes; the include-clean gate, run by hand — no CI builds the C++ tree (§8) — own `build-hygiene/` |
...
- **Sanitizers: NOT BUILT. No CI builds or tests the C++ tree, and no sanitizer has ever run here — there is no sanitizer box to tick.** CI does exist, and naming it is what makes this checkable: `.github/workflows/atx-db.yml`, a GitHub Actions job `focused-quality` on `ubuntu-latest`, `on: push`/`pull_request` **path-filtered to `atx-db/**`**, running `compileall` + `ruff` + `mypy --strict` + a focused `pytest`. It never invokes CMake, ctest, clang-cl or a sanitizer, and a C++ change cannot trigger it, so it gates nothing in this document. It is on `main` and on other branches but **not on this one**, which is exactly why a worktree-scoped check misses it — so **check refs, not your worktree**. Swept over every ref rather than a hand-picked few (`git for-each-ref --format='%(refname)'` piped into `git ls-tree -r --name-only` per ref, plus `git log --all --diff-filter=A` for anything since deleted): that workflow is the sole CI file present on any ref *and* the sole one ever added in this repository's history, and its blob is byte-identical on every ref that carries it. Run that sweep for the current list rather than trusting a written one — the refs carrying it change, the properties do not. No `.gitlab-ci.yml`, `azure-pipelines.yml`, `.circleci/`, `Jenkinsfile` or `.buildkite/` anywhere. If you ever restate this, re-run the all-refs sweep — a three-ref one is how the earlier version of this sentence came to be false. And no sanitizer build: `CMakePresets.json` declares no sanitizer preset — enumerate them with `cmake --list-presets` plus `cmake --list-presets=test` rather than from a list copied here, because a hand-kept name list is the same artifact as a hand-kept count (an earlier version of this sentence listed nine and silently omitted the `ninja-fast` test preset). **Both commands hide `hidden: true` presets — which includes `_base`, the one that actually sets `ATX_USE_PCH: ON` for every preset inheriting it — so read `CMakePresets.json` directly when a hidden preset is the subject.** Root `CMakeLists.txt` declares no `option(ATX_*SAN*)`; `cmake/` holds only the install/version/config templates; `scripts/atx-build.ps1` has no sanitizer verb or flag. The `-fsanitize` wiring in the tree is third-party and inert on every count found: the vendored `atx-core/third-party/databento-cpp/cmake/Sanitizers.cmake` (its `DATABENTO_ENABLE_{ASAN,TSAN,UBSAN}` default OFF, and nothing first-party sets them), and spdlog's `SPDLOG_SANITIZE_ADDRESS`, observed `OFF` in every configured build tree's `CMakeCache.txt`. This entry records an unimplemented intention so it stops reading as a gate that passed; it is not licence to write UB, which §0 forbids on its own authority.
