#!/usr/bin/env python
"""Build the point-in-time q-factor-style quarterly ROE feature."""
...
def main() -> int:
    ...
    parser.add_argument("--maximum-earnings-age-days", type=int, default=200)
    ...
    options = QuarterlyRoeOptions(
        start_date=args.start_date,
        end_date=args.end_date,
        maximum_earnings_age_days=args.maximum_earnings_age_days,
        minimum_names_per_date=args.minimum_names_per_date,
        winsor_limit=args.winsor_limit,
        run_id=args.run_id,
    )
    with DuckDBStore(args.db_path) as store:
        rows = refresh_quarterly_roe_values(store, options)
