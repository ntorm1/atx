#!/usr/bin/env python
"""Build the governed point-in-time large R&D-increase event factor."""
...
from atx_db.rd_increase import FACTOR_ID, RdIncreaseOptions, refresh_rd_increase_values
...
def main() -> int:
    ...
    parser.add_argument("--growth-threshold",type=float,default=0.05)
    parser.add_argument("--maximum-reporting-age-days",type=int,default=550)
    parser.add_argument("--maximum-history-days",type=int,default=1300)
    ...
    options = RdIncreaseOptions(
        start_date=args.start_date,end_date=args.end_date,
        intensity_threshold=args.intensity_threshold,growth_threshold=args.growth_threshold,
    ...
    with DuckDBStore(args.db_path) as store:
        count = refresh_rd_increase_values(store,options)
        coverage = store.con.execute(
            """SELECT count(*) AS rows,count(DISTINCT security_id) AS securities,
                      ...
                      sum(CASE WHEN raw_value=1 THEN 1 ELSE 0 END) AS event_rows,
