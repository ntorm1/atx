#!/usr/bin/env python
"""Build the PIT price-deflated quarterly earnings-acceleration factor."""
...
from atx_db.earnings_acceleration import (
    ...
    refresh_earnings_acceleration_values,
)
...
def main() -> int:
    ...
    parser.add_argument("--maximum-signal-age-days", type=int, default=150)
    parser.add_argument("--maximum-absolute-acceleration", type=float, default=10.0)
    ...
    options = EarningsAccelerationOptions(
        start_date=args.start_date,
        end_date=args.end_date,
        maximum_signal_age_days=args.maximum_signal_age_days,
        maximum_absolute_acceleration=args.maximum_absolute_acceleration,
        minimum_names_per_date=args.minimum_names_per_date,
        winsor_limit=args.winsor_limit,
        run_id=args.run_id,
    )
    with DuckDBStore(args.db_path) as store:
        rows = refresh_earnings_acceleration_values(store, options)
