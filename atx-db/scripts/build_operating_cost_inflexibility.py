#!/usr/bin/env python
"""Build the governed point-in-time operating-cost-inflexibility factor."""
...
def main() -> int:
    ...
    parser.add_argument("--maximum-history-days",type=int,default=2200)
    parser.add_argument("--minimum-five-year-span-days",type=int,default=1300)
    parser.add_argument("--maximum-five-year-span-days",type=int,default=1650)
    parser.add_argument("--maximum-sdoc",type=float,default=5.0)
    ...
    options = OperatingCostInflexibilityOptions(
        start_date=args.start_date,end_date=args.end_date,
        maximum_reporting_age_days=args.maximum_reporting_age_days,
        maximum_history_days=args.maximum_history_days,
        minimum_five_year_span_days=args.minimum_five_year_span_days,
        maximum_five_year_span_days=args.maximum_five_year_span_days,
        maximum_sdoc=args.maximum_sdoc,
        minimum_names_per_date=args.minimum_names_per_date,
        winsor_limit=args.winsor_limit,run_id=args.run_id,
    )
    with DuckDBStore(args.db_path) as store:
        count = refresh_operating_cost_inflexibility_values(store,options)
