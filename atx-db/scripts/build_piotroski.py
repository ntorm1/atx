#!/usr/bin/env python
"""Build point-in-time Piotroski financial-strength factors."""
...
from atx_db.piotroski import FACTOR_IDS, PiotroskiOptions, refresh_piotroski_values
...
def main() -> int:
    ...
    parser.add_argument(
        "--minimum-high-book-to-market-names-per-date", type=int, default=10
    )
    parser.add_argument("--high-book-to-market-percentile", type=float, default=0.80)
    ...
    options = PiotroskiOptions(
        ...
        minimum_high_book_to_market_names_per_date=(
            args.minimum_high_book_to_market_names_per_date
        ),
        high_book_to_market_percentile=args.high_book_to_market_percentile,
        run_id=args.run_id,
    )
    with DuckDBStore(args.db_path) as store:
        rows = refresh_piotroski_values(store, options)
        placeholders = ",".join("?" for _ in FACTOR_IDS)
        coverage = store.con.execute(
            f"""SELECT factor_id,count(*) AS rows,
                ...
                WHERE source=? AND factor_id IN ({placeholders})
