#!/usr/bin/env python
"""Build governed point-in-time low Beneish manipulation-risk scores."""
...
def main() -> int:
    ...
    parser.add_argument("--maximum-absolute-m-score", type=float, default=100.0)
    ...
    options = BeneishMScoreOptions(
        ...
        maximum_absolute_m_score=args.maximum_absolute_m_score,
        minimum_names_per_date=args.minimum_names_per_date,
        winsor_limit=args.winsor_limit,
        run_id=args.run_id,
    )
    with DuckDBStore(args.db_path) as store:
        count = refresh_beneish_m_score_values(store, options)
