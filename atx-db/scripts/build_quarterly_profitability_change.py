#!/usr/bin/env python
"""Build PIT same-quarter change in quarterly operating profitability."""
...
from atx_db.quarterly_profitability_change import (
    ...
    refresh_quarterly_profitability_change_values,
)
...
def main() -> int:
    ...
    parser.add_argument("--maximum-absolute-change", type=float, default=10.0)
    ...
    options = QuarterlyProfitabilityChangeOptions(
        ...
        maximum_absolute_change=args.maximum_absolute_change,
        minimum_names_per_date=args.minimum_names_per_date,
        winsor_limit=args.winsor_limit,
        run_id=args.run_id,
    )
    with DuckDBStore(args.db_path) as store:
        rows = refresh_quarterly_profitability_change_values(store, options)
    ...
    print(
        json.dumps(
            {"rows_materialized": rows, "coverage": coverage.to_dict(orient="records")},
            default=str,
            indent=2,
        )
    )
