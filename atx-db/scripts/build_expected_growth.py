#!/usr/bin/env python
"""Build the point-in-time q5 published-slope expected-growth proxy."""
...
from atx_db.expected_growth import (
    ...
    refresh_expected_growth_values,
)
...
def main() -> int:
    ...
    parser.add_argument("--minimum-annual-age-days", type=int, default=120)
    parser.add_argument("--maximum-annual-age-days", type=int, default=550)
    ...
    options = ExpectedGrowthOptions(
        start_date=args.start_date,
        end_date=args.end_date,
        minimum_annual_age_days=args.minimum_annual_age_days,
        maximum_annual_age_days=args.maximum_annual_age_days,
        minimum_names_per_date=args.minimum_names_per_date,
        winsor_limit=args.winsor_limit,
        run_id=args.run_id,
    )
    with DuckDBStore(args.db_path) as store:
        rows = refresh_expected_growth_values(store, options)
