...
def test_formula_registry_migration_bodies_are_idempotent(tmp_store):
    ...
    from atx_db.migrations import _formula_registry_schema_catalog, _formula_registry_indexes
...
def test_seed_formula_registry_rejects_duplicate_formula_code_in_seed(tmp_path):
    ...
    with pytest.raises(ValueError, match="[Dd]uplicate"):
        from atx_db.formula_library import _dedupe_formula_rows

        _dedupe_formula_rows(rows)


def test_default_formula_registry_seed_path_exists():
    from atx_db.formula_library import SEED_PATH
...
def test_formula_registry_seed_csv_is_stdlib_parseable():
    ...
    """
    from atx_db.formula_library import SEED_PATH, SEED_COLUMNS as MODULE_SEED_COLUMNS
...
def test_s6_0_catalog_rows_load_and_parse_through_closed_interpreter():
    ...
    from atx_db.formula_library import eval_operand_term, load_ratio_formula_rows, parse_operand_expression
...
def test_migration_0148_body_upserts_existing_s6_0_catalog_rows(tmp_store):
    from atx_db.migrations.bodies_0148_0151 import _pf3_s6_ratio_formula_catalog_expansion
...
def test_eval_operand_term_key_shape():
    from atx_db.formula_library import eval_operand_term
...
def test_resolve_composite_evaluator_maps_known_keys_to_named_functions():
    from atx_db.formula_library import (
...
def test_resolve_composite_evaluator_unknown_key_raises_key_error():
    ...
    from atx_db.formula_library import resolve_composite_evaluator
...
def test_parse_operand_expression_splits_numerator_and_denominator():
    from atx_db.formula_library import parse_operand_expression
...
class TestRoeDupont3Way:
    def test_known_value_telescopes_to_roe(self):
        from atx_db.formula_library import roe_dupont_3way
...
class TestRoeDupont5Way:
    def test_known_value_telescopes_to_roe(self):
        from atx_db.formula_library import roe_dupont_5way
...
class TestTotalAccruals:
    def test_known_value(self):
        from atx_db.formula_library import total_accruals
...
class TestWorkingCapitalAccruals:
    def test_known_value(self):
        from atx_db.formula_library import working_capital_accruals
...
class TestCashConversionCycleComponents:
    def test_days_sales_outstanding_known_value(self):
        from atx_db.formula_library import days_sales_outstanding
    ...
    def test_days_inventory_outstanding_known_value(self):
        from atx_db.formula_library import days_inventory_outstanding
    ...
    def test_days_payables_outstanding_known_value(self):
        from atx_db.formula_library import days_payables_outstanding
    ...
    def test_cash_conversion_cycle_none_when_component_unusable(self):
        from atx_db.formula_library import cash_conversion_cycle
...
class TestPerShareComposites:
    def test_eps_basic_known_value(self):
        from atx_db.formula_library import eps_basic
    ...
    def test_eps_diluted_known_value(self):
        from atx_db.formula_library import eps_diluted
    ...
    def test_tangible_book_value_per_share_known_value(self):
        from atx_db.formula_library import tangible_book_value_per_share
...
class TestMontierCScore:
    def test_known_value_two_flags_trigger(self):
        from atx_db.formula_library import montier_c_score
