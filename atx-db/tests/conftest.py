"""pytest configuration and shared fixtures for the atx_db test suite.

Run tests from atx-db/ so the src-layout package resolves consistently.
...
# Keep direct pytest invocations consistent with an editable install.
...
_SCHEMA_FINGERPRINT_FILES = (
    ...
    _SOURCE_ROOT / "atx_db" / "fundamental_statements.py",
    _SOURCE_ROOT / "atx_db" / "parity.py",
)
...
def _schema_fingerprint() -> str:
    ...
    paths = sorted(
        (
            *_SCHEMA_FINGERPRINT_FILES,
            *(_SOURCE_ROOT / "atx_db" / "migrations").glob("*.py"),
            *(_SOURCE_ROOT / "atx_db" / "seeds").glob("*.csv"),
        ),
        key=lambda p: p.relative_to(_PROJECT_ROOT).as_posix(),
    )
    for path in paths:
