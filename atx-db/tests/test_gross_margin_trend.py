...
from atx_db.gross_margin_trend import (
    FACTOR_ID,
    SOURCE_NAME,
    GrossMarginTrendOptions,
    compute_gross_margin_trend_rows,
)
            ...
            [
                {
                    ...
                    "gross_margin": 0.4 + trend * quarter,
                }
...
def test_gross_margin_trend_orientation_and_lineage() -> None:
    result = compute_gross_margin_trend_rows(
        pd.DataFrame([_row("LOW", -0.02), _row("MID", 0.0), _row("HIGH", 0.02)]),
        GrossMarginTrendOptions(minimum_names_per_date=2, winsor_limit=0.0),
...
def test_gross_margin_trend_rejects_nonfinite_and_extreme_values() -> None:
    result = compute_gross_margin_trend_rows(
        pd.DataFrame(
            [
                _row("GOOD", 0.02),
        ...
        GrossMarginTrendOptions(minimum_names_per_date=1, winsor_limit=0.0),
    )
    assert result["security_id"].tolist() == ["GOOD", "GOOD_2"]


def test_gross_margin_trend_definition_is_governed(tmp_store) -> None:
