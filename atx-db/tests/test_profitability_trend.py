...
from atx_db.profitability_trend import (
    FACTOR_ID,
    SOURCE_NAME,
    ProfitabilityTrendOptions,
    compute_profitability_trend_rows,
)
...
def _row(security_id: str, trend: float) -> dict[str, object]:
    history = [
        {
            "factor_value_id": f"{security_id}-q{quarter}",
            "period_end": f"202{3 + quarter // 4}-{3 * (quarter % 4 + 1):02d}-30",
            "raw_value": trend * quarter,
        }
        for quarter in range(8)
    ]
    return {
        "decision_factor_value_id": f"{security_id}-decision",
...
def test_profitability_trend_orientation_and_lineage() -> None:
    result = compute_profitability_trend_rows(
        pd.DataFrame([_row("LOW", -0.02), _row("MID", 0.0), _row("HIGH", 0.02)]),
        ProfitabilityTrendOptions(minimum_names_per_date=2, winsor_limit=0.0),
...
def test_profitability_trend_rejects_nonfinite_and_extreme_values() -> None:
    result = compute_profitability_trend_rows(
        pd.DataFrame(
            [
                _row("GOOD", 0.02),
        ...
        ProfitabilityTrendOptions(minimum_names_per_date=1, winsor_limit=0.0),
    )
    assert result["security_id"].tolist() == ["GOOD", "GOOD_2"]


def test_profitability_trend_definition_is_governed(tmp_store) -> None:
    ...
    assert tmp_store.con.execute(
        """SELECT dependency_name FROM factor_dependency_edges
           WHERE factor_id=?""",
        [FACTOR_ID],
    ).fetchall() == [("profitability_quarterly_gross_profitability_lagged_assets",)]
