...
def test_held_apply_lock_fails_fast(tmp_store):
    from atx_db.migrations import acquire_apply_lock, apply_pending_migrations
...
def test_apply_lock_release_is_holder_scoped(tmp_store):
    from atx_db.migrations import acquire_apply_lock, release_apply_lock
...
def test_apply_lock_blocks_second_connection(built_warehouse):
    from atx_db.migrations import acquire_apply_lock
...
def test_backup_database_and_restore_roundtrip(built_warehouse, tmp_path):
    from atx_db.connection import DuckDBStore
    from atx_db.migration_admin import backup_database, checkpoint, restore_database
...
def test_governed_forward_migration_on_populated_pre_s2_db(tmp_path, monkeypatch):
    from atx_db.connection import DuckDBStore
    from atx_db.migration_admin import run_governed_migrations, verify_schema
    import atx_db.migrations as migrations
...
def test_recover_from_wal_failure_restores_backup_and_reapplies(built_warehouse, tmp_path):
    from atx_db.connection import DuckDBStore
    from atx_db.migration_admin import backup_database, checkpoint, recover_from_wal_failure
...
def test_enforce_backup_retention_prunes_only_registered_completed_backups(built_warehouse, tmp_path):
    from atx_db.connection import DuckDBStore
    from atx_db.migration_admin import (
        backup_database,
