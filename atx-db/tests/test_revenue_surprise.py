...
from atx_db.revenue_surprise import (
    ...
    compute_revenue_surprise_rows,
)


def _row(security_id: str, surprise: float) -> dict[str, object]:
    return {
        ...
        "statement_point_id": f"{security_id}-current",
        "period_end": dt.date(2024, 9, 30),
        "available_at": dt.datetime(2024, 11, 1),
        "accession_number": f"{security_id}-current",
        "revenue": 120.0,
        "prior_statement_point_id": f"{security_id}-prior",
        "prior_period_end": dt.date(2023, 9, 30),
        "prior_available_at": dt.datetime(2023, 11, 1),
        "prior_accession_number": f"{security_id}-prior",
        "prior_revenue": 100.0,
        "seasonal_revenue_change": 20.0,
        "history_observations": 8,
        "historical_drift": 5.0,
        "historical_std": 7.5,
...
def test_revenue_surprise_orientation_and_drift_lineage() -> None:
    rows = compute_revenue_surprise_rows(
        pd.DataFrame([_row("A", 2.0), _row("B", -1.0)]),
        RevenueSurpriseOptions(minimum_names_per_date=2, winsor_limit=0.0),
    ...
    assert lineage["currency_policy"] == "USD_only"
    assert lineage["standardization"]["history_observations"] == 8
    assert lineage["standardization"]["historical_drift"] == pytest.approx(5.0)


def test_revenue_surprise_is_governed_by_migration(tmp_store) -> None:
    assert tmp_store.con.execute(
        ...
        True,
        "atx-db PIT standardized unexpected revenue v1",
    )
