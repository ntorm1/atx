"""Point-in-time standardized unexpected quarterly earnings.

The signal is the first-filed diluted EPS change from the same quarter one year
earlier, divided by the trailing standard deviation of prior seasonal EPS changes.
"""
...
SOURCE_NAME = "atx-db PIT standardized unexpected EPS v1"
FACTOR_ID = "earnings_standardized_unexpected_eps"
FACTOR_NAME = "PIT standardized unexpected diluted EPS"
...
def load_earnings_surprise_inputs(
    store: DuckDBStore,
    options: EarningsSurpriseOptions | None = None,
) -> pd.DataFrame:
    """Resolve first-filed seasonal EPS surprises at monthly decision dates."""

    options = options or EarningsSurpriseOptions()
    history = int(options.maximum_history_observations)
    if history < options.minimum_history_observations or options.minimum_history_observations < 2:
        raise ValueError("earnings-surprise history bounds are invalid")
    ...
    sql = f"""
        ...
        seasonal_candidates AS (
            SELECT
                ...
                prior.eps_diluted AS prior_eps_diluted,
                row_number() OVER (
        ...
        unexpected AS (
            SELECT *, eps_diluted - prior_eps_diluted AS unexpected_eps
        ...
        standardized AS (
            SELECT
                *,
                count(unexpected_eps) OVER history_window AS history_observations,
                stddev_samp(unexpected_eps) OVER history_window AS historical_std
            FROM unexpected
        ...
        signals AS (
            SELECT *, unexpected_eps / nullif(historical_std, 0) AS sue
            FROM standardized
...
def _lineage(row: pd.Series) -> str:
    return json_dumps(
        {
            "formula": "(eps_diluted_q - eps_diluted_q_minus_4) / prior_seasonal_change_std",
            ...
            "seasonal_prior": {
                ...
                "eps_diluted": row["prior_eps_diluted"],
            },
            "standardization": {
                "unexpected_eps": row["unexpected_eps"],
...
def compute_earnings_surprise_rows(
    inputs: pd.DataFrame,
    options: EarningsSurpriseOptions | None = None,
) -> pd.DataFrame:
    """Convert PIT SUE inputs into monthly winsorized cross-sectional scores."""

    options = options or EarningsSurpriseOptions()
    if inputs is None or inputs.empty:
        return pd.DataFrame(columns=_OUTPUT_COLUMNS)
    required = {"security_id", "symbol", "trade_date", "price_available_at", "sue"}
    missing = sorted(required.difference(inputs.columns))
    if missing:
        raise ValueError(f"earnings surprise inputs missing columns: {missing}")
    rows = inputs.copy()
    ...
    rows["sue"] = pd.to_numeric(rows["sue"], errors="coerce")
    rows = rows.dropna(subset=["security_id", "as_of_date", "available_at", "sue"])
    rows = rows[rows["sue"].map(math.isfinite)].copy()
    ...
    rows["raw_value"] = rows["sue"].astype(float)
    rows = winsorize(
    ...
    rows["input_ids_json"] = json_dumps(["metric:eps_diluted_quarterly"])
...
def refresh_earnings_surprise_values(
    store: DuckDBStore,
    options: EarningsSurpriseOptions | None = None,
) -> int:
    """Materialize the monthly first-filed SUE factor."""

    options = options or EarningsSurpriseOptions()
    store.initialize()
    rows = compute_earnings_surprise_rows(load_earnings_surprise_inputs(store, options), options)
    ...
    with store.transaction():
        ...
        if not rows.empty:
            insert_frame(store, rows, "fundamental_factor_values", "earnings_surprise_values_insert")
    return len(rows)
