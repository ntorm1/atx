...
def _pf3_s9_price_liquidity_factor_definitions(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    conn.execute(
        ...
        WHERE fd.declared_in = 'atx_db.factors.cross_domain.price_liquidity_specs'
        """
    )
    conn.execute(
        """
...
def _pf3_s9_estimate_13f_factor_definitions(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    conn.execute(
        ...
        LEFT JOIN (
          ...
          ON edges.factor_id = fd.factor_id
        WHERE fd.declared_in = 'atx_db.factors.cross_domain.estimate_13f_specs'
        """
    )
    conn.execute(
        """
...
def _pf3_s9_short_insider_factor_definitions(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    conn.execute(
        ...
        LEFT JOIN (
          ...
          ON edges.factor_id = fd.factor_id
        WHERE fd.declared_in = 'atx_db.factors.cross_domain.short_insider_specs'
        """
    )
    conn.execute(
        """
...
def _pf3_s9_unified_cross_domain_factor_surface(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    conn.execute(
        ...
        SELECT
            ...
            fd.factor_name,
            CASE
                WHEN fd.declared_in = 'atx_db.factors.cross_domain.price_liquidity_specs' THEN 'price_liquidity'
                WHEN fd.factor_id LIKE 'estimate_%' THEN 'estimate_revision'
        ...
        WHERE fd.declared_in IN (
            'atx_db.factors.cross_domain.price_liquidity_specs',
            'atx_db.factors.cross_domain.estimate_13f_specs',
            'atx_db.factors.cross_domain.short_insider_specs'
        )
        """
    )
    conn.execute(
