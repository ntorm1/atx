"""Point-in-time quarterly operating profits-to-lagged-assets factor."""
...
SOURCE_NAME = "atx-db PIT quarterly operating profitability v1"
FACTOR_ID = "profitability_quarterly_operating_profitability_lagged_assets"
FACTOR_NAME = "PIT quarterly operating profits-to-lagged assets"
...
class QuarterlyOperatingProfitabilityOptions:
...
def load_quarterly_operating_profitability_inputs(
    store: DuckDBStore,
    options: QuarterlyOperatingProfitabilityOptions | None = None,
) -> pd.DataFrame:
    """Resolve the latest visible quarterly operating profit and lagged assets."""

    options = options or QuarterlyOperatingProfitabilityOptions()
    ...
    sql = f"""
        WITH quarterly_components AS (
            SELECT
                ...
                max(available_at) FILTER (WHERE canonical_metric = 'sga')
                    AS sga_available_at,
                arg_max(value, (available_at, revision_sequence, statement_point_id))
                ...
                max(available_at) FILTER (WHERE canonical_metric = 'rd_expense')
                    AS rd_expense_available_at
            FROM fundamental_statement_points
            WHERE canonical_metric IN (
                    'revenue', 'cogs', 'gross_profit', 'sga', 'rd_expense'
                  )
              AND unit = 'USD'
        ...
        quarterly_profit AS (
                ...
                CASE
                    WHEN revenue IS NOT NULL AND cogs IS NOT NULL
                        THEN greatest(
                            ...
                            cogs_available_at,
                            sga_available_at,
                            coalesce(rd_expense_available_at, TIMESTAMP '1900-01-01')
                        )
                    ELSE greatest(
            ...
            WHERE sga IS NOT NULL
              AND ((revenue IS NOT NULL AND cogs IS NOT NULL)
...
def _lineage(
    ...
    return json_dumps(
        {
            "method": "hou_xue_zhang_quarterly_operating_profitability_pit",
            "formula": (
                "(revenue-cogs-sga+coalesce(rd_expense,0))"
            ...
            "published_method_adaptations": {
                ...
                "rd_expense": "Missing R&D is zero, matching the published definition.",
            ...
            "quarterly_statement": {
                ...
                "sga": {"id": row["sga_id"], "value": row["sga"]},
                "rd_expense": {
                    "id": row.get("rd_expense_id"),
                    "value": row.get("rd_expense"),
                    "missing_replaced_with_zero": bool(pd.isna(row.get("rd_expense"))),
                },
            ...
            "quarterly_operating_profitability": row["raw_value"],
        }
    )


def compute_quarterly_operating_profitability_rows(
    inputs: pd.DataFrame,
    options: QuarterlyOperatingProfitabilityOptions | None = None,
) -> pd.DataFrame:
    """Compute, winsorize, and standardize quarterly operating profitability."""
    ...
    if missing:
        raise ValueError(f"quarterly operating profitability inputs missing columns: {missing}")
    ...
    for column in ("resolved_gross_profit", "sga", "rd_expense", "total_assets"):
    ...
    rows = rows[
        ...
        & rows["sga"].map(math.isfinite)
        & rows["total_assets"].map(math.isfinite)
    ].copy()
    rows["operating_profit"] = (
        rows["resolved_gross_profit"] - rows["sga"] + rows["rd_expense"].fillna(0.0)
    )
    rows["quarterly_operating_profitability"] = (
        rows["operating_profit"] / rows["total_assets"]
    )
    rows = rows[rows["quarterly_operating_profitability"].map(math.isfinite)].copy()
    rows = rows[
        rows["quarterly_operating_profitability"].abs()
    ...
    rows["raw_value"] = rows["quarterly_operating_profitability"]
    rows = winsorize(
        rows,
...
def refresh_quarterly_operating_profitability_values(
    store: DuckDBStore,
    options: QuarterlyOperatingProfitabilityOptions | None = None,
) -> int:
    """Materialize point-in-time quarterly operating profitability."""

    options = options or QuarterlyOperatingProfitabilityOptions()
    store.initialize()
    rows = compute_quarterly_operating_profitability_rows(
        load_quarterly_operating_profitability_inputs(store, options), options
    )
    ...
    with store.transaction():
        ...
        if not rows.empty:
            insert_frame(
                ...
                "quarterly_operating_profitability_insert",
            )
    return len(rows)
