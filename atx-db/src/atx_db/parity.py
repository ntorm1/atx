...
PROVIDER_PARITY_ROWS: tuple[ProviderParityRow, ...] = (
    ...
    ProviderParityRow(
        ...
        factors_or_fields=("manager metadata", "CUSIP holdings", "shares/value", "discretion", "put/call", "voting authority", "filer CIK alias / entity-resolution rollup"),
        open_substitute="SEC bulk Form 13F ZIP files, identifier-resolution evidence, PIT derived manager/security ownership feature tables, and a filer CIK alias / subadvisor / M&A-continuity entity-resolution spine.",
        warehouse_tables=(
    ...
    ProviderParityRow(
        provider="FINRA",
        provider_domain="OTC Transparency off-exchange volume",
        warehouse_domain="offexchange_transparency",
        reference_tables=("FINRA OTC Transparency weeklySummary", "monthlySummary", "blockSummary", "FINRA Query API otcMarket group"),
        institutional_grain="Off-exchange ATS and non-ATS volume per security, venue (MPID), and summary period.",
        institutional_keys=("symbol", "mpid", "venue_class", "period_type", "summary_start_date"),
        pit_fields=("summary_start_date", "available_at", "finra_last_update_date", "as_of_date"),
        factors_or_fields=("ATS dark-pool volume", "non-ATS member-firm volume", "trade count", "ATS market-share percent", "ATS venue count", "restatement flags", "period quality report"),
        open_substitute="FINRA OTC Transparency weekly/monthly/block off-exchange volume ingested (injectable CSV; production source is the FINRA Query API otcMarket group, OAuth2, tiered publication delay) into offexchange_volume + offexchange_venue, with a derived offexchange_security_period ATS-share rollup, a period-level offexchange_quality_report control surface, and a security-joined view.",
        warehouse_tables=("offexchange_venue", "offexchange_volume", "offexchange_security_period", "offexchange_quality_report", "v_offexchange_security_period"),
        parity_status="partial",
        limitations="Schema, restatement handling (is_latest/restatement_seq), as-of APIs, quality checks, lake exports, and a dedicated offexchange_quality_report table are built and reproduce the FINRA blueprint fixture exactly, but the default DB is empty until FINRA files are injected; the live OAuth2 FINRA Query API connector is not yet wired.",
        next_gap="Wire the FINRA Query API (OAuth2 client-credentials, otcMarket group) connector with the 10 GB/mo cap guard and tiered publication-delay period computation, add monthly/block ingestion cadence, and backfill the ~2014-present weekly history.",
        source_urls=(
            "https://www.finra.org/finra-data/browse-catalog/equity-short-interest/otc-transparency",
            "https://developer.finra.org/products/query-api",
        ),
    ),
    ProviderParityRow(
    ...
    ProviderParityRow(
        ...
        institutional_keys=("security_or_vendor_id", "broker_id", "analyst_id", "measure_code", "period_end", "announce_date", "activation_date"),
        pit_fields=("announce_date", "activation_date", "revision_date", "stop_date", "available_at", "source_loaded_at"),
