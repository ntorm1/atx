"""Composite of standardized earnings surprise and post-SEC-filing reaction."""
...
from .filing_reaction import FACTOR_ID as REACTION_FACTOR_ID
from .warehouse import insert_frame, json_dumps

SOURCE_NAME = "atx-db PIT earnings confirmation composite v1"
FACTOR_ID = "earnings_sue_filing_confirmation"
FACTOR_NAME = "PIT SUE and SEC filing-reaction confirmation"
...
class EarningsConfirmationOptions:
...
def load_earnings_confirmation_inputs(
    store: DuckDBStore,
    options: EarningsConfirmationOptions | None = None,
) -> pd.DataFrame:
    """Load the latest finite SUE/reaction pair on identical security/date keys."""

    options = options or EarningsConfirmationOptions()
    predicates = ["factor_id IN (?, ?)", "is_latest_revision"]
    params: list[object] = [SUE_FACTOR_ID, REACTION_FACTOR_ID]
    ...
    return store.con.execute(
        ...
        paired AS (
            SELECT
                ...
                max(factor_value_id) FILTER (WHERE factor_id = ?) AS sue_factor_value_id,
                max(available_at) FILTER (WHERE factor_id = ?) AS sue_available_at,
                max(input_lineage_json) FILTER (WHERE factor_id = ?) AS sue_lineage_json,
                max(value) FILTER (WHERE factor_id = ?) AS reaction_value,
                max(factor_value_id) FILTER (WHERE factor_id = ?) AS reaction_factor_value_id,
                max(available_at) FILTER (WHERE factor_id = ?) AS reaction_available_at,
                max(input_lineage_json) FILTER (WHERE factor_id = ?) AS reaction_lineage_json
            FROM ranked
            WHERE revision_rank = 1
            GROUP BY security_id, as_of_date
        )
        SELECT *, greatest(sue_available_at, reaction_available_at) AS decision_available_at
        FROM paired
        WHERE sue_value IS NOT NULL AND reaction_value IS NOT NULL
        ORDER BY as_of_date, security_id
        """,
        [
            *params,
            SUE_FACTOR_ID, SUE_FACTOR_ID, SUE_FACTOR_ID, SUE_FACTOR_ID,
            REACTION_FACTOR_ID, REACTION_FACTOR_ID, REACTION_FACTOR_ID, REACTION_FACTOR_ID,
        ],
    ).df()
...
def _lineage(row: pd.Series) -> str:
    return json_dumps(
        {
            "method": "equal_weight_intersection",
            "sue": {
                "factor_value_id": row["sue_factor_value_id"],
                "value": row["sue_value"],
                "available_at": row["sue_available_at"],
                "lineage": _decode(row["sue_lineage_json"]),
            },
            "filing_reaction": {
                "factor_value_id": row["reaction_factor_value_id"],
                "value": row["reaction_value"],
                "available_at": row["reaction_available_at"],
                "lineage": _decode(row["reaction_lineage_json"]),
            },
        }
    )


def compute_earnings_confirmation_rows(
    inputs: pd.DataFrame,
    options: EarningsConfirmationOptions | None = None,
) -> pd.DataFrame:
    """Equal-weight the two standardized inputs, then standardize the intersection."""

    options = options or EarningsConfirmationOptions()
    if inputs is None or inputs.empty:
        return pd.DataFrame(columns=_OUTPUT_COLUMNS)
    required = {
        "security_id", "symbol", "as_of_date", "decision_available_at",
        "sue_value", "reaction_value",
    }
    missing = sorted(required.difference(inputs.columns))
    if missing:
        raise ValueError(f"earnings confirmation inputs missing columns: {missing}")
    rows = inputs.copy()
    ...
    for column in ("sue_value", "reaction_value"):
        rows[column] = pd.to_numeric(rows[column], errors="coerce")
    rows = rows.dropna(
        subset=["security_id", "as_of_date", "available_at", "sue_value", "reaction_value"]
    )
    rows = rows[
        rows["sue_value"].map(math.isfinite) & rows["reaction_value"].map(math.isfinite)
    ].copy()
...
def refresh_earnings_confirmation_values(
    store: DuckDBStore,
    options: EarningsConfirmationOptions | None = None,
) -> int:
    """Materialize the SUE/filing-reaction confirmation factor."""

    options = options or EarningsConfirmationOptions()
    store.initialize()
    rows = compute_earnings_confirmation_rows(
        load_earnings_confirmation_inputs(store, options), options
    )
    ...
    with store.transaction():
        ...
        if not rows.empty:
            insert_frame(store, rows, "fundamental_factor_values", "earnings_confirmation_insert")
    return len(rows)
