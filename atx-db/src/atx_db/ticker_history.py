...
def _vendor_identifier_value(symbol: str, value: object) -> str:
    ...
    if raw in {"", "0", "<NA>", "nan", "NaN", "NaT", "None"}:
...
def _normalize_chunk(chunk: pd.DataFrame, options: TickerHistoryOptions) -> pd.DataFrame:
    required_columns = PRICE_PROJECTION_COLUMNS if options.price_projection_only else SOURCE_COLUMNS
...
def disambiguate_vendor_collisions(store: DuckDBStore, source: str = SOURCE_NAME) -> int:
    ...
    if not nonprimary.empty:
        ...
        nonprimary["id_value"] = [
            ...
            for sk, vid in zip(
                nonprimary["symbol_key"], nonprimary["vendor_security_id"], strict=True
            )
        ]
        ...
        try:
            with store.transaction():
                ...
                raw_exists_row = con.execute(
                    "SELECT COUNT(*) FROM duckdb_tables() "
                    "WHERE schema_name = 'main' AND table_name = 'tbltickerhistory_daily'"
                ).fetchone()
                raw_exists = bool(raw_exists_row and raw_exists_row[0])
                if raw_exists:
                    con.execute(
                        """
    ...
    def load(self, store: DuckDBStore, options: TickerHistoryOptions) -> DatasetLoadResult:
        ...
        for raw_chunk in _iter_raw_chunks(effective_options):
            ...
            if chunks_seen == 1 or chunks_seen % 10 == 0:
                LOGGER.info(
                    "scanned ticker-history chunk %s (%s matching chunks, %s rows loaded)",
                    chunks_seen,
                    chunks_with_matches,
                    total_rows,
                )
