"""Point-in-time abnormal inventory growth relative to revenue growth."""
...
SOURCE_NAME = "atx-db PIT abnormal inventory growth v1"
FACTOR_ID = "investment_low_abnormal_inventory_growth"
FACTOR_NAME = "PIT low abnormal inventory growth"
...
class AbnormalInventoryGrowthOptions:
...
def load_abnormal_inventory_growth_inputs(
    store: DuckDBStore,
    options: AbnormalInventoryGrowthOptions | None = None,
) -> pd.DataFrame:
    """Attach latest visible inventory facts to governed YoY revenue-growth rows."""

    options = options or AbnormalInventoryGrowthOptions()
    ...
    return store.con.execute(
        ...
        inventory_facts AS (
            SELECT
                ...
                period_end,
                value AS inventory,
                statement_point_id AS inventory_id,
                available_at AS inventory_available_at,
                revision_sequence
            FROM fundamental_statement_points
            WHERE canonical_metric = 'inventory'
              AND unit = 'USD'
        ...
        current_candidates AS (
            SELECT
                b.*,
                i.inventory AS current_inventory,
                i.inventory_id AS current_inventory_id,
                i.inventory_available_at AS current_inventory_available_at,
                row_number() OVER (
                    PARTITION BY b.revenue_growth_factor_value_id
                    ORDER BY i.inventory_available_at DESC NULLS LAST,
                             i.revision_sequence DESC NULLS LAST,
                             i.inventory_id DESC NULLS LAST
                ) AS current_rank
            FROM revenue_base b
            LEFT JOIN inventory_facts i
              ON i.security_id = b.security_id
             AND i.period_end = b.current_period_end
             AND i.inventory_available_at <= b.decision_available_at
        ),
        ...
        prior_candidates AS (
            SELECT
                b.*,
                i.inventory AS prior_inventory,
                i.inventory_id AS prior_inventory_id,
                i.inventory_available_at AS prior_inventory_available_at,
                row_number() OVER (
                    PARTITION BY b.revenue_growth_factor_value_id
                    ORDER BY i.inventory_available_at DESC NULLS LAST,
                             i.revision_sequence DESC NULLS LAST,
            ...
            FROM current_inventory b
            LEFT JOIN inventory_facts i
              ON i.security_id = b.security_id
             AND i.period_end = b.prior_period_end
             AND i.inventory_available_at <= b.decision_available_at
        )
...
def _lineage(row: pd.Series, options: AbnormalInventoryGrowthOptions) -> str:
    return json_dumps(
        {
            "method": "inventory_growth_minus_revenue_growth_yoy_pit",
            "formula": (
                "(inventory_t/inventory_t_4-1)"
            ...
            "research_contract": {
                ...
                "missing_inventory_imputed": False,
...
def compute_abnormal_inventory_growth_rows(
    inputs: pd.DataFrame,
    options: AbnormalInventoryGrowthOptions | None = None,
) -> pd.DataFrame:
    """Compute, orient, winsorize, and standardize abnormal inventory growth."""
    ...
    if missing:
        raise ValueError(f"abnormal inventory growth inputs missing columns: {missing}")
    ...
    timestamp_columns = (
        ...
        "current_inventory_available_at",
        "prior_inventory_available_at",
    )
    ...
    rows["raw_value"] = rows["inventory_growth"] - rows["revenue_growth"]
    rows = rows[
        rows["inventory_growth"].map(math.isfinite)
        & rows["raw_value"].map(math.isfinite)
        & (
            rows["inventory_growth"].abs()
            <= options.maximum_absolute_inventory_growth
        )
    ...
    rows["input_ids_json"] = json_dumps(
        [f"factor:{REVENUE_GROWTH_FACTOR_ID}", "metric:inventory"]
    )
...
def refresh_abnormal_inventory_growth_values(
    store: DuckDBStore,
    options: AbnormalInventoryGrowthOptions | None = None,
) -> int:
    """Materialize the point-in-time abnormal inventory-growth factor."""

    options = options or AbnormalInventoryGrowthOptions()
    store.initialize()
    rows = compute_abnormal_inventory_growth_rows(
        load_abnormal_inventory_growth_inputs(store, options), options
    )
    ...
    with store.transaction():
        ...
        if not rows.empty:
            insert_frame(
                ...
                "abnormal_inventory_growth_insert",
            )
    return len(rows)
