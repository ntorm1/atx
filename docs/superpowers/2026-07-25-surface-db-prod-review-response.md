...
| P-02 | Blocker | The per-date split calls the table seam once per symbol, each call rescanning every row. Architectural: needs sorted-span slicing at the seam |
...
| F-03 | Medium | `--max-absent` has no automated CLI test. Correct, and already the named first task of the next sprint — the repo has **no CLI test harness for either surface-db tool**, so this is a new test target with a process-spawning fixture |
