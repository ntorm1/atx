...
- Builds ONLY via `.\scripts\atx-build.ps1 build atx-vol-tests --parallel 8` (never bare cmake/ninja for the monorepo; the standalone python project's documented cmake route is the accepted exception). ctest via `.\scripts\atx-build.ps1 -Ctest -R "<regex>"`.
- **Replay-parity gate (the accuracy contract of this sprint):** the reference run is `run_sp100_strangle_backtest.py --db C:\atx-scratch\surface-db\sp100-2026 --from 2026-01-02 --to 2026-07-31 --exclude BK` at the pre-sprint commit; its `track.tsv` is the golden. After EVERY task in Phases 1–3, re-run and `memcmp` `track.tsv` — any byte difference is a failed task, no exceptions, no tolerance. (Timing keys live in `tearsheet.tsv`/stdout, never `track.tsv`, so the file is byte-stable by design.)
- These four gates stay green after every task: `SurfaceDbDispersionBacktest.BitIdenticalAcrossThreadCounts` (bit_cast-exact), `PricedSurface.EvaluateBatchLanedGreeksPackCompositionInvariant`, `SpyDispersionPnl.*`, `UnpricedTolerance.*`.
- Bit-identical across thread counts is a standing engine contract; new scheduling must keep pack/tile membership book-determined, never thread-determined.
- **Forbidden (economics-changing — rejected with evidence, do not re-propose):**
  - AL preset change `al_fast_opts{7,16,4}` → `al_bulk_opts` (1.81× but different marks; american.hpp:102-106).
...
### Task 1: Solve-ledger + phase visibility in the driver
...
- [ ] Bind + emit. Run the pinned file-scoped test lane. PASS.
- [ ] Run the reference YTD replay once; record the ledger totals in the sprint workspace as `baseline-ledger.txt`. Sanity: `sl_al_boundary_solves / 141 sessions / 12,726 uniques ≈ 6.2`.
- [ ] Replay-parity gate (byte-compare `track.tsv`). Commit.
...
- [ ] Implement, gate, commit. (No replay-parity needed — tooling only.)

---

## Phase 1 — Scheduling: close the 65% → ~90% utilization gap (all BIT-IDENTICAL)
...
- [ ] **Replay-parity gate** (both topologies must produce byte-identical `track.tsv` — that IS the certification). Commit.
...
## Phase 2 — Pack occupancy: the CPU payload (BIT-IDENTICAL, gated)
...
Task 8's entire bit-identity claim rests on "a 1-lane pack returns exactly what a 4-lane pack returns" (laned_greek_run.hpp:22-26). Today's test pins that for same-T packs. Extend it: packs mixing distinct T (and distinct surfaces per the kernel's per-lane `S,K,T,σ,r,q` signature, simd/american_boundary_batch.hpp:98-118) must return per-lane results bit-identical to 1-lane evaluation. **If this test cannot be made green, Task 8 is dead — STOP and report; do not absorb a tolerance.**

- [ ] Write the extended test against the CURRENT kernel (no production change). Expected: PASS (the kernels are documented pack-composition invariant). If it fails, the sprint re-plans here.
- [ ] Commit the test.
...
- [ ] Implement. Gates: Task 7's mixed-T invariance, `BitIdenticalAcrossThreadCounts`, full portfolio/surface suites.
...
- [ ] Red-first per item where testable, implement, gates, **replay-parity**, commit.

---

## Phase 3 — Load lane + driver wall (BIT-IDENTICAL)
...
- [ ] Implement (a), then (b). Gates + **replay-parity** (also compare a settlement-heavy window — wings do get hit at expiry intrinsics).
...
- [ ] Red-first pytest (session_ts equals the engine's ts on a fixture db; prefetch_depth round-trips), implement, gates, **replay-parity**, commit.

---

## Phase 4 — DECISION GATE (ULP class — requires explicit human sign-off before any work)

### Task 12: `/arch:AVX2` `_core` — measure, then decide

The only remaining lever after Phases 1-3 is FMA contraction in the scalar AL boundary solver (`/arch:AVX2` on the whole build). This is **ULP-class**: it flips `kFmaContraction`, every `golden_close()` loses its byte-exact branch (57 call sites), expected drift ~13 ULP/value, ~5.5e-13 relative on accumulated NAV — and the repo has **zero ISA-paired baselines**, so the gain is unmeasured (plausibly single-digit %). The replay-parity gate as defined above would FAIL by construction.

- [ ] Prereqs: Task 3 (ISA-aware compare gate) done; Phases 1-3 landed and measured.
- [ ] Build a side-by-side `_core` with `CXXFLAGS`/`CFLAGS` env injection (NOT `cmake.define` — CMakePresets.json:81 warns it drops `/EHsc`); measure the YTD replay; quantify the track.tsv drift (max abs/rel per column).
- [ ] Present numbers to the human: wall gained vs accuracy band moved vs baseline-regeneration cost (K8: full `-avx2-` baseline set under the quiet-host protocol). **Do not land without sign-off.** If declined, delete the experiment and record the measurement.
...
|---|---|---|---|
| Task 4 dynamic greek tiles | bit-identical | ~flat | −15-35% of step wall |
| Task 5 P-core pinning | bit-identical | ~flat | −10-30% (measure; may lose) |
| Task 6 substrate retention | bit-identical | −3-6% | more (serial section) |
| Task 8 mixed-T packs | bit-identical | −30-45% | follows CPU |
| Task 9 redundancy kills | bit-identical | −1-2% | small |
| Task 10 lazy heavy curves | bit-identical | −15-25 s abs | small |
| Task 11 pre-pass fast path | bit-identical | — | −2.8 s abs |
| Task 12 /arch:AVX2 | **ULP** | unknown | unknown |

Compounded conservative estimate for Phases 1-3: **CPU 1,810 → ~900-1,100 s; wall 175 → ~60-95 s**, byte-identical `track.tsv` throughout. Stretch (all levers at top of range + Phase 4 approved): wall < 60 s.

## Self-review notes

- Spec coverage: "why it takes so long" = the measured baseline + cost model section (94% book-marking, 5-solve bundle, 1-lane packs, 65% utilization); "minimize while maintaining same economic accuracy" = the replay-parity byte gate on every task, the bit-identical/ULP/forbidden taxonomy, and Phase 4's human gate.
...
- Sequencing honors hard dependencies: Task 4 before Task 8 (tile determinism prerequisite), Task 7 before Task 8 (gate first), Task 3 before Task 12 (measurable at all), Task 1 before everything (the ledger is the judge).
