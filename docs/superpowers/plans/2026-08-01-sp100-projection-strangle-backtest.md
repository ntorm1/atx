...
- Produces: no new public API. Behavior change under `ExcludeAndReport` ONLY:
  ...
  4. **Entry skip on missing index (strategy-side, not engine-side):** `DispersionStrangleConfig` gains `bool skip_entry_on_missing_index{false}`. When true and the INDEX leg cannot resolve on an entry step because its surface is absent from the snapshot (NotFound on the index symbol — distinguish from other resolution errors, which still propagate), the strategy enters NO cohort that step and continues; existing cohorts are unaffected. Default false preserves today's hard error. Motivation: the real 2026 corpus has SPY fit-rejected on 18 of 140 sessions (arb-violating quotes at the snapshot minute — unrecoverable by refit), so a YTD run must drop those 18 entry days rather than abort. Wire the flag through `make_dispersion_strangle_spec` (onto whatever spec/strategy mechanism implements it — if the DSL has no natural home, a `StrategySpec`-level `skip_entry_on_unresolvable_index` consumed by the declarative interpreter is acceptable). Add a test: 3-session synthetic corpus, index surface absent on session 2, flag on → run succeeds with no session-2 cohort and session 2's basket-only entry NOT taken (no cohort at all); flag off → run errors.
...
3. **Verify:** `--phase verify` over each window; then enumerate absences (`atx-vol-surface-db.exe partitions`). OUTCOME RECORDED 2026-08-01: the recovery pass (`--deep-selection --retry-disabled`, both DST windows) recovered 0 of the 310 fit-failed cells — they are risk-surface arbitrage rejections (butterfly/calendar/inversion) of the underlying quotes, not solver-tier failures. The scratch db therefore matches prod's census exactly: SPY absent 18/140, BK 46 (44 provider from 2026-05-21), AMT 47, CMCSA 28, SYK 25, etc. The run proceeds on this corpus using Task 2's tolerances: `skip_entry_on_missing_index` covers the 18 SPY entry days, ExcludeAndReport covers mid-life holes, `--exclude BK` covers the permanent outage.
