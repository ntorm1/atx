...
const functionNames = [
  ...
  'bootstrapPathError', 'bootstrapReportError',
]
...
const bootstrapReportSchema = Function(`
  ...
  ${bootstrapSchemaSource}; return BOOTSTRAP_REPORT
`)()

function schemaErrors(schema, value, path = '$') {
  if (schema.oneOf) {
    const branches = schema.oneOf.map(item => schemaErrors(item, value, path))
    return branches.filter(errors => errors.length === 0).length === 1 ? [] : [`${path} must match exactly one branch: ${JSON.stringify(branches)}`]
  }
  const errors = []
...
test('Bootstrap StructuredOutput schema admits typed BLOCKED and rejects malformed DONE reports', () => {
  ...
  assert.deepEqual(schemaErrors(bootstrapReportSchema, done), [])
  for (const mutate of [
    ...
    assert.notDeepEqual(schemaErrors(bootstrapReportSchema, malformed), [])
  }
  ...
  assert.deepEqual(schemaErrors(bootstrapReportSchema, blocked), [])
  ...
  for (const suffix of ['; git status --short', ' (cwd C:\\atx-wt\\pool-13)']) {
    ...
    assert.notDeepEqual(schemaErrors(bootstrapReportSchema, malformedBlocked), [])
    assert.match(validators.bootstrapReportError(malformedBlocked, expectedFor(malformedBlocked)), /diagnostics invalid/u)
  }

  assert.match(workflow, /oneOf: \[/u)
  assert.match(workflow, /command: \{ type: 'string', pattern: '\^\[\^;&\|`\(\)\\\\r\\\\n\]\+\$' \}/u)
  assert.match(workflow, /DONE requires a committed SHA, nonempty success evidence/u)
