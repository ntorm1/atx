...
// would widen a frozen surface for zero callers. `detail/` is where atx-vol
// already keeps header-only internals shared across translation units, and this
// header includes nothing but <cmath>/<cstdint>, so it sits below every consumer:
