...
//
// ── Thread-safety ──────────────────────────────────────────────────────────
// Every evaluator here is a pure function of `CStarParams` (a value type). The
// projection entries mutate only the slices the caller hands them, so they are
// safe to call concurrently over disjoint slice spans.
//
// The standalone per-family container that stacked CStar slices by ascending T
// was demoted to `detail/legacy_cstar_surface.hpp` by S4-T21 / plan 4.4 — the
// canonical pipeline is CurveSurface -> PricedSurface / PricedSurfaceView ->
// SurfaceSet.
