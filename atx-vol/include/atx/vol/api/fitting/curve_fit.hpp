...
#include "atx/vol/api/fitting/calib.hpp"          // FitObs, DeAmAuditDiagnostics
#include "atx/vol/api/fitting/deamer.hpp"         // CarryDiagnostics
#include "atx/vol/api/fitting/parity.hpp"         // ParityReport
#include "atx/vol/api/fitting/surface_parity.hpp" // SliceContext, SurfaceParityInputs
#include "atx/vol/api/core/types.hpp"          // Result
#include "atx/vol/api/marketdata/universe.hpp"       // Underlying
#include "atx/vol/api/fitting/vol_curve.hpp"      // CurveSurface, CurveConfig
...
struct CurveSurfaceReport {
  ...
  // Slices whose k-coverage refusal was absorbed by the opt-in parametric
  // demotion (`CalibOpts::per_slice_uncovered_parametric`) and which therefore
  // SERVE an eSSVI curve instead of vanishing from the surface. Such a slice is
  // reported `ExpiryFitOutcome::FittedFallbackCurve` and does NOT increment
  // `n_slices_uncovered` — the refusal is absorbed, so counting it as a drop
