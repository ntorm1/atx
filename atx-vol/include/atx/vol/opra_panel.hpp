...
struct OpraPanel {
  QuoteFrame frame;         // ready for data_install (flat r yield-curve pillar)
...
//   3. Assemble the frame: uid = the underlying (or first root), snapshot_iso,
//      snapshot_ts_ns from the first `ts`, a single flat yield-curve pillar
//      {T=1, r=spec.r}, the kept rows, then build_uid_list + build_expiry_inputs.
//   4. Spot: spec.spot_override if > 0, else imply from the earliest expiry with
//      a co-terminal call/put pair (both mids > 0) via imply_forward_atm_pcp,
//      then implied_spot = F * exp(-r * T_front). Unavailable if no pair exists.
//
// @return InvalidArgument on a read failure or a frame-assembly (install
