...
| P5 | pending | | hold: session.cpp may collide with G1 consumer audit — dispatch after G1 lands |
| P6 | pending | | hold: counter gate wants build-counters — dispatch after G1 frees it. RIDER: validate P7 bench counters-ON compile + cnt_clenshaw_sweeps 2353→1343 column |
...
| W3 | in-flight | | folded into G2 agent as step 0 (single american.* owner) |
...
| G2 | in-flight | | dispatched with W3 step 0 (build/ dir, single american.* owner). Watch item: SpyDispersionPnl.DailyDeltaHedgeBandsNetDelta flagged once in build/ during concurrent G1 WIP (green in rel-avx2 at 79abd68) — triage after G1 lands if it persists on a clean tree. |
| G4 | pending | | |
