...
| P5 | **done** | 49b797e | american_price_cached_ladder: collapse_T_plane once per endpoint + 2D eval_plane per strike (~n_T× less Clenshaw work). evaluate_ladder AND fair_value_ladder wired (CachedPriceBatch mirrors ColdPriceBatch flush). Economic parity 2.84e-17/K (bit-identity impossible: T is middle collapse axis, reorder inherent). 2 justified repins EXPECT_DOUBLE_EQ→1e-12·K. bulk.cpp NOT wired (per-lane resolve, unfloored PriceOnly, dT partial needed — documented in-file; F12 LOW). Counter gate CorrectionCache.LadderBatchClenshawSweepCountIsStrikeCountIndependent (scalar=n vs batch=1, ON-sweep pending). 110+192 pass. SpyDispersionPnl PASSES clean rel-avx2 (4.4e-15). |
| P6 | in-flight (PM ruling issued) | | RECLASSIFIED bit-identity→economic parity (|ΔIV|<1e-9 pin; measured 1.2e-11 put cold path): rtsafe converges |dx|<1e-7 not machine root, seed shifts last digits; calib.cpp warm chain + WarmStartResultInvariantToSeed(1e-6) are shipped precedent. Batch warm_start ships (counter 65→42 residual evals, 35%). de_americanize_chain change must EARN default-ON via cached-route (DeAmOptions.caches) counter — cold route measured 147→147 zero; if cached also ~zero → DELETE (no default-OFF dead plumbing), documented as measured-and-rejected. |
| P7 | **done** | 32161ad | 5 bench rows (cached-Newton IV, carry legacy/warm A/B, correction build put/call) + name-coverage ctest. Always-on sl_* ledger columns cite wins from shipping binary: 474→139 carry solves (P2), 96 build solves, 983 IV iters/200. cnt_* columns light up under counters build (ON-mode compile pending P6 rider). Fixture fix: call-side build carry q=0.02 (q≤0 short-circuits American call). |
