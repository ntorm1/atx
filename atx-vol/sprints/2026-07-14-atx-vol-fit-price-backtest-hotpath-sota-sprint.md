...
| W1.1 Wire SIMD batch | complete (benchmark-gated) | `f1dd5901bd4826b6693494c3d4f3274ed37f49a0` | Public price, value/vega, Greeks, and eSSVI span APIs dispatch to AVX2 with scalar invalid/tail patching. Release real-time medians at 16,384 rows: value/vega 1.53x, Greeks 1.60x, eSSVI 2.76x. The existing AVX2 IV inverter measured 0.92x and therefore remains internal/scalar at the public boundary until W5.4 makes it profitable. |
| W1.2 Binary-search surface bracket | complete | `f1dd5901bd4826b6693494c3d4f3274ed37f49a0` | Contiguous maturity mirror + `lower_bound`; one private, unforgeable bracket per equal-T batch run; exact nodes collapse to one slice. Focused scalar/batch equivalence is green. |
| W1.3 Fast query tier by default in backtest | complete (safe reusable-sweep gate; global authoritative default rejected) | `d3aa2853ed6d3dc5234b5d95646b56bd319f3e86` | Tier-keyed `SnapshotCache`, `ReuseOnly`, and RepresentativeFast/CarryBank preparation are plumbed end to end. Fast tiers are screening-only with cold confirmation and forced-cold marks/Greeks/P&L. A literal global authoritative default is rejected by W0 economics and one-shot preload cost; prebuilt Representative screen+cold remains the 8.84x accepted route. |
| W1.4 Fix `term_rates_` detection | complete | `f1dd5901bd4826b6693494c3d4f3274ed37f49a0` | Slice rates decode once; finite scale-relative DF classification removes query-time logs on flat surfaces without misclassifying tiny genuine term discounts. |
| W1.5 Drop diagnostic double-work on pinned surfaces | complete | `f1dd5901bd4826b6693494c3d4f3274ed37f49a0` | Mark-only pinned non-eSSVI paths elide unconsumed parity scoring and correction-cache construction. Risk/Quote, Mark bid/ask floors, qualified-corpus quality rules, auto/eSSVI fallback, and fast serving retain required evidence/caches. Scored-vs-elided archive bytes compare equal. |
| W2.1 Bind σ-independent geometry once | complete | `e7d5ebb78208b8eda62c5f71a9661c4571c2e0c4` | Static `(T,r,q)` geometry is retained across sigma residuals; only `sigma*sqrt(t)` is rebound. Fast/accurate exact counter tests prove `192+32/price` and `528+96/price` exp calls respectively. |
| W2.2 Reusable `AloPricer` | complete | `e7d5ebb78208b8eda62c5f71a9661c4571c2e0c4` | Allocation-free `reset`, uninitialized inactive geometry storage, and a re-entrancy-safe TLS lease remove owning-state allocation after a thread's first cold AL inversion. |
| W2.3 Fewer + faster carry pairs | complete (selector coverage follow-up in W3.2) | `e7d5ebb78208b8eda62c5f71a9661c4571c2e0c4` | Ordinary/Latency/Balanced carry caps are five; carry uses an independent fast AL preset and `1e-4` inner-IV tolerance. A controlled MU A/B proved 12 pairs retains eSSVI while five selects a much better-fit SVI on fewer observations; W3.2 must make that comparison use a common population and coverage floor. |
...
**Wave-1 verification and benchmark gate (`f1dd590`):** strict Debug and Release builds pass with `/W4 /WX`; 71 focused tests pass in each configuration and cover public/low-level SIMD, nonfinite/domain/side/tail semantics, bracket/rate equivalence, fit-admission fail-closed behavior, real-SPY pinned fits, and corpus consumers. The low-level 16,384-row Release medians are 1.53x for value/vega (`786 us` → `515 us`), 1.60x for Greeks (`757 us` → `472 us`), and 2.76x for eSSVI (`36.2 us` → `13.1 us`). IV measured slower (`5.292 ms` → `5.725 ms`, 0.92x), so the public IV adapter deliberately stays scalar. Batch overlap is now rejected at the public boundary rather than relying on order-dependent in-place behavior; no in-repo producer aliases these buffers.

**Wave-2 verification and benchmark gate (`e7d5ebb`):** strict Debug/counters-on and Release builds pass with `/W4 /WX`; 96 impacted Debug tests, 21 counter/adversarial Debug tests, 78 impacted Release tests, two accuracy-panel determinism CTests, eight real-SPY accuracy/corpus tests, and the controlled MU selector A/B pass. The corrected one-operation real-OPRA SPY row falls from `1.031 s` to `487 ms` end to end (2.12x): fit `997.814 ms` → `465.599 ms`, observation de-Am `408.624 ms`, carry `56.342 ms`, and Prices-only value `26.951 ms` → `18.140 ms`. The 200-contract American-IV microbench reaches a `100.035 ms` cold median (`~2.13k inversions/s`) and `74.697 ms` warm median (`~2.56k/s`); this is still far below the structural target and confirms W3.1 is required rather than claiming victory from local hoists.

**Wave-2 bounded economic gate:** accuracy-panel mark routing now explicitly retains parity diagnostics so W1.5's legitimate mark-only elision cannot silently zero the oracle. On three SPY snapshots plus the first ten real universe names, 13/13 build successfully with zero Greek failures. Against the same baseline rows, half-spread pass rate is unchanged at `94.952%`, half-tick improves `58.534%` → `59.736%`, the conservative economic-gate rate is unchanged at `43.149%`, and calendar violations fall `1,140` → `1,114`. MU's five-pair selector route moves eSSVI → SVI, improves in-band fraction `72.559%` → `96.343%` and vol RMSE `0.07379` → `0.00509`, but scores fewer observations (`4,867` → `2,567`); W3.2 therefore owns common-population and coverage-floor enforcement before the final full-panel decision. The scored sample SHA-256 is `2F05CD3BEF103B5866FB81F27250575AF9BFE9B61C5ECA75B006B6DD2518CED4`.
