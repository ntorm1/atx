...
ATX_FORCE_INLINE int greeks_block(const double* F, const double* K,
                                  const double* T, const double* sigma,
                                  const double* r, const double* df,
                                  const Side* side, std::size_t i,
                                  BlockOut& b) noexcept {
...
struct AoSSink {
    ...
    ATX_FORCE_INLINE void scalar(std::size_t idx) noexcept {
        const Black76Greeks bg = black76_greeks(F[idx], K[idx], T[idx],
                                                sigma[idx], r[idx], df[idx],
                                                side[idx]);
        g[idx] = bg.greeks;
        px[idx] = bg.price;
    }
...
struct SoASink {
    ...
    const Side* side;
    GreeksBatchSoA out;
    ATX_FORCE_INLINE void store(std::size_t idx, const Greeks& gg,
                                double p) noexcept {
        if (out.delta) out.delta[idx] = gg.delta;
        if (out.gamma) out.gamma[idx] = gg.gamma;
        if (out.vega) out.vega[idx] = gg.vega;
        if (out.theta) out.theta[idx] = gg.theta;
        if (out.rho) out.rho[idx] = gg.rho;
        if (out.vanna) out.vanna[idx] = gg.vanna;
        if (out.volga) out.volga[idx] = gg.volga;
        if (out.charm) out.charm[idx] = gg.charm;
        if (out.price) out.price[idx] = p;
    }
    ATX_FORCE_INLINE void vec(std::size_t idx, int j, const BlockOut& b) noexcept {
        if (out.delta) out.delta[idx] = b.dl[j];
        if (out.gamma) out.gamma[idx] = b.gm[j];
        if (out.vega) out.vega[idx] = b.vg[j];
        if (out.theta) out.theta[idx] = b.th[j];
        if (out.rho) out.rho[idx] = b.rh[j];
        if (out.vanna) out.vanna[idx] = b.vn[j];
        if (out.volga) out.volga[idx] = b.vl[j];
        if (out.charm) out.charm[idx] = b.cm[j];
        if (out.price) out.price[idx] = b.pr[j];
    }
    ATX_FORCE_INLINE void scalar(std::size_t idx) noexcept {
        const Black76Greeks bg = black76_greeks(F[idx], K[idx], T[idx],
                                                sigma[idx], r[idx], df[idx],
...
template <class Sink>
ATX_FORCE_INLINE void greeks_batch_drive(const double* F, const double* K,
                                         const double* T, const double* sigma,
                                         const double* r, const double* df,
                                         const Side* side, std::size_t n,
                                         Sink sink) noexcept {
    std::size_t i = 0;
    for (; i + 4 <= n; i += 4) {
        BlockOut b;
