...
ATX_FORCE_INLINE __m256d sr2017_seed_pd(__m256d price, __m256d F, __m256d K,
                                        __m256d T, __m256d df, __m256d eps,
    ...
    const __m256d eps_eff =
        _mm256_blendv_pd(eps, _mm256_sub_pd(zero, eps), itm);
    ...
    const __m256d R =
        _mm256_sub_pd(_mm256_mul_pd(two, Q), _mm256_mul_pd(eps_eff, f_m1));
    ...
    const __m256d term2 =
        _mm256_div_pd(_mm256_mul_pd(_mm256_mul_pd(two, sum_h), inner), f);
    ...
    const __m256d Cc =
        _mm256_mul_pd(_mm256_mul_pd(coef, Q_minus), _mm256_mul_pd(Cf1, Cf2));

    // disc = max(B² + 4AC, 0); β = 2C / (B + √disc) (stable, avoids B−√ cancel).
    __m256d disc =
        _mm256_fmadd_pd(four, _mm256_mul_pd(A, Cc), _mm256_mul_pd(B, B));
    ...
    const __m256d gamma =
        _mm256_div_pd(_mm256_sub_pd(zero, log_pd(beta_safe)), kp);
    ...
    const __m256d sqrt_v =
        _mm256_add_pd(_mm256_sqrt_pd(gpy), _mm256_sqrt_pd(gmy));
...
ATX_FORCE_INLINE __m256d iv_halley_step_pd(__m256d sigma, __m256d price,
                                           __m256d F, __m256d K, __m256d df,
                                           __m256d is_put, __m256d sqrtT,
                                           __m256d lnFK,
    ...
    const __m256d call_pr = _mm256_mul_pd(
        df, _mm256_sub_pd(_mm256_mul_pd(F, Nd1), _mm256_mul_pd(K, Nd2)));
    ...
    const __m256d vega =
        _mm256_mul_pd(_mm256_mul_pd(df, F), _mm256_mul_pd(phi1, sqrtT));
    const __m256d volga = _mm256_div_pd(
        _mm256_mul_pd(vega, _mm256_mul_pd(d1, d2)), sigma); // vega·d1·d2/σ
...
ATX_FORCE_INLINE void iv_evaluate_pd(__m256d sigma, __m256d price, __m256d F,
                                     __m256d K, __m256d df, __m256d is_put,
                                     __m256d sqrtT, __m256d lnFK,
                                     const double* coefs, __m256d& d1o,
                                     __m256d& d2o, __m256d& vegao,
...
ATX_FORCE_INLINE void patch_scalar(const double* price, const double* F,
...
void implied_vol_batch_avx2(const double* price, const double* F,
    ...
    for (; i + 4 <= n; i += 4) {
        ...
        eps_buf[0] = (side[i + 0] == Side::Put) ? -1.0 : 1.0;
        eps_buf[1] = (side[i + 1] == Side::Put) ? -1.0 : 1.0;
        eps_buf[2] = (side[i + 2] == Side::Put) ? -1.0 : 1.0;
        eps_buf[3] = (side[i + 3] == Side::Put) ? -1.0 : 1.0;
        ...
        const __m256d bad = _mm256_or_pd(
            _mm256_cmp_pd(Tv, zero, _CMP_LE_OQ),
            _mm256_or_pd(
                _mm256_cmp_pd(Fv, zero, _CMP_LE_OQ),
        ...
        iv_evaluate_pd(sigma, Pv, Fv, Kv, dfv, is_put, sqrtT, lnFK, coefs, d1,
                       d2, vega, resid);
        ...
        const __m256d wing_bad =
            _mm256_or_pd(_mm256_cmp_pd(abs_d1, wing, _CMP_GT_OQ),
                         _mm256_cmp_pd(abs_d2, wing, _CMP_GT_OQ));
        const __m256d abs_resid = _mm256_andnot_pd(abs_mask, resid);
        const __m256d not_conv =
            _mm256_cmp_pd(abs_resid, resid_tol, _CMP_GE_OQ);
        const __m256d vfloor = _mm256_mul_pd(
            _mm256_mul_pd(vfloor_coef, _mm256_add_pd(Fv, Kv)), dfv);
        const __m256d ill_cond = _mm256_cmp_pd(vega, vfloor, _CMP_LT_OQ);
        const __m256d not_valid = _mm256_andnot_pd(seed_valid, all_ones);
        const __m256d nan_out =
            _mm256_or_pd(_mm256_cmp_pd(sigma, sigma, _CMP_UNORD_Q),
