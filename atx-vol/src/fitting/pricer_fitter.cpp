...
#include "atx/vol/api/pricing/black76.hpp"          // black76_price (independent publication oracle)
#include "atx/vol/api/fitting/calib.hpp"            // build_observations_european
#include "atx/vol/api/fitting/correction.hpp"       // AmericanCorrectionCaches (cached inversion hot path)
#include "atx/vol/api/fitting/deamer.hpp"           // resolve_chain_forward
#include "fitting/convex_recovery.hpp"  // strict-recovery bridge (admission-rejection rung)
...
void merge_session_failure_context(const SessionDiagnostics &diagnostics,
  ...
  if (diagnostics.n_carry_skipped_expiries > 0 || diagnostics.n_audit_starved_expiries > 0 ||
      diagnostics.n_prep_starved_expiries > 0 || diagnostics.n_prep_uncovered_expiries > 0 ||
      diagnostics.n_carry_fallback_expiries > 0) {
    // §5.2 + Decision B: expiries dropped by the carry gate, starved by the fit
    // audit, starved by PREPARATION (T6), refused by the k-COVERAGE predicate
    // (T6d), OR admitted with a term-structure-fallback carry all surface as
    // the one publish-with-Degraded reason, CarryGap
    // (decide_risk_surface_admission); combined with any other failure it
    // still rejects.
    //
    // T6 added the preparation term, and it is the BIGGEST of the five: 30.2%
    ...
    // to close, arriving through a different field. T6d closed the last member
    // of the class: the ConvexDense coverage refusal (PrepUncovered) drops an
    // expiry into the same truthful-drop lane as starvation and was likewise
    // invisible here.
