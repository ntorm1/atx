...
def test_schema_py_not_stale_vs_cpp_header():
    ...
    * header + generator absent   -> assert ``_schema.py`` does not ADVERTISE a
    ...
    header = atx_vol / "src" / "storage" / "run_archive_schema.hpp"
...
_CPP_HEADER = _ATX_VOL / "src" / "storage" / "run_archive_schema.hpp"
...
# Main's compact aggregate gates complement the feature branch's focused
# accessor and corruption cases.
def test_header_and_sections(archive):
    ...
    assert h.run_identity_hash == 0xABCDEF
    assert h.schema_hash == GOLDEN_SCHEMA_HASH
    assert archive.sections == ["backtest", "meta"]
...
@pytest.mark.parametrize("mutate,label", [
    (lambda data: data.__setitem__(24, data[24] ^ 1), "schema hash drift"),
    (lambda data: data.__setitem__(0, ord("Z")), "bad magic"),
    (lambda data: data.__setitem__(16, data[16] ^ 0xFF), "header CRC"),
])
def test_framing_corruption_is_rejected_at_open(tmp_path, mutate, label):
    data = _fixture_bytes()
    mutate(data)
    path = tmp_path / "corrupt.atxrun"
    path.write_bytes(data)
    with pytest.raises(ValueError):
        ra.RunArchive.open(str(path))
...
def test_from_bytes_decodes_strings_and_keeps_views_readonly():
    ...
    nav = sec.f64("nav")
    assert nav.base is not None
    assert nav.flags.writeable is False
    with pytest.raises(ValueError):
        nav[0] = 1.0
