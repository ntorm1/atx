...
_ARCHIVE_DATA = Path(__file__).resolve().parent / "data" / "runarchive"
_COMMITTED_ARCHIVE = _ARCHIVE_DATA / "dispersion_paired.atxrun"
_GOLDEN_TSV = _ARCHIVE_DATA / "backtest_paired_golden.tsv"
_EXPECTED_SECTIONS = [
...
_live_e2e = pytest.mark.skipif(bool(_SKIP_REASON), reason=_SKIP_REASON)
...
@_live_e2e
def test_live_io_reads_final_nav_from_archive(run_dir: Path):
...
# Main's committed archive is the fast, build-independent half of this module.
# These tests deliberately are not covered by `_live_e2e`: a missing example
# driver must not hide regressions in the Python reader/report path.
@pytest.fixture(scope="module")
def archive():
    ar = RunArchive.open(str(_COMMITTED_ARCHIVE))
    yield ar
    ar.close()
...
def test_io_reads_final_nav_from_archive():
    ...
    result, meta, extra = io.read_backtest_archive(str(_COMMITTED_ARCHIVE))
    assert result.size() == _EXPECTED_DATES
    assert f"{float(result.nav[-1]):.10g}" == _EXPECTED_FINAL_NAV


def test_archive_reproduces_the_golden_backtest_tsv(archive):
    header, rows = parity._read_tsv(str(_GOLDEN_TSV))
    section = archive.section("backtest")
    assert len(rows) == _EXPECTED_DATES
    assert [row["date"] for row in rows] == list(section.dict("date"))
    assert [int(row["ts_ns"]) for row in rows] == list(section.i64("ts_ns"))

    numeric = [column for column in header if column not in ("date", "ts_ns")]
    assert numeric, "golden carries no numeric columns"
    for column in numeric:
        assert list(section.f64(column)) == [
...
def test_parity_report_renders_from_the_archive(tmp_path):
    out = tmp_path / "parity.html"
    stats = parity.build_parity_report_from_archive(
        str(_COMMITTED_ARCHIVE), str(out)
    )
...
@pytest.mark.slow
@_live_e2e
def test_cli_publishes_the_archive_and_drops_the_loose_tsvs(run_dir: Path):
    assert (run_dir / "run.atxrun").is_file()
    for gone in ("backtest.tsv", "reconciliation.tsv", "contract_marks.tsv"):
        assert not (run_dir / gone).exists(), f"{gone} should no longer be written"
...
@pytest.mark.slow
@_live_e2e
def test_cli_dump_reproduces_backtest_tsv_byteshape(run_dir: Path):
    proc = subprocess.run(
        [str(_EXE), "runarchive", "dump", str(run_dir), "backtest", "--tsv"],
        env={
            **os.environ,
            "PATH": str(_BIN) + os.pathsep + os.environ.get("PATH", ""),
        },
        capture_output=True,
        timeout=120,
    )
    assert proc.returncode == 0, proc.stderr.decode("utf-8", "replace")
    assert proc.stdout == _GOLDEN_TSV.read_bytes()
