...
void run_symbol(const std::string& sym_lc, const std::string& sym_uc) {
  ...
  std::printf("\n== %s ==  spot %.2f  |  %zu expiries  %zu strikes  (%zu legs)\n",
              sym_uc.c_str(), board->spot(), U.chains.size(), n_strikes,
              chain->size());
  ...
  if (fitter.selection().has_value()) {
    ...
    for (const auto& s : sel.scores) {
      std::printf("    %-13s  OOS in-band %6.2f%%   vw %6.2f%%   avg-dof %5.1f   "
                  ...
                  s.n_slices ? static_cast<double>(s.dof_sum) /
                                   static_cast<double>(s.n_slices)
                             : 0.0,
                  s.n_holdout, s.n_slices);
    }
  }

  const auto sc = testkit::price_in_band(fitter.surface()->session(), U,
                                         board->spot(), board->r);
  std::printf("  served price-in-band: pxCLN %.2f%% (%zu/%zu)   pxALL %.2f%%\n",
              sc.px_clean, sc.n_clean_in, sc.n_clean, sc.px_all);
}
...
int main() {
  std::printf(
      "vol_breadth_bench — auto-select curve per underlying (real OPRA, "
      "2026-06-05T19:55Z)\n");
  run_symbol("spy", "SPY");
  run_symbol("xom", "XOM");
  std::printf(
