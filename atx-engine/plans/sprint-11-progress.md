...
Next: S11-4 (store cluster-panel record + index) persists this result; then S11-5+ wire the labels as
cluster-aware alpha / risk grouping. S11-3 binds to the as-built `cluster(const MatX&, ClusterConfig)`
and `rmt_clean(const MatX&, f64 q, RmtConfig)` signatures.
