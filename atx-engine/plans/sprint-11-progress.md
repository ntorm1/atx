...
Next: S11-6+ validation + GICS head-to-head (loading the persisted panel via the S11-4 contract and the
cluster field via the S11-5 helpers above). S11-3 binds to the as-built `cluster(const MatX&,
ClusterConfig)` and `rmt_clean(const MatX&, f64 q, RmtConfig)` signatures.
