...
> **As-built v2 (2026-06-13, this sprint's kickoff recon).** The S6 `cost/` layer has since been **merged into `main`** (`e23daf8 merge: S6 cost-calibration & capacity`), and S7 is being built on a fresh `feat/atx-engine-book` branch cut from `main` (concurrent efforts now isolated in git worktrees — no shared-branch hazard). The post-merge deltas **D1–D6** are recorded in [`sprint-7-progress.md`](sprint-7-progress.md) (the as-built ledger) and **supersede §0.1 and §0.9 below**: D1 — the cost API (`cost::cost_aware_knobs`, `cost::capacity_point`, `cost::should_trade`, `cost::CalibratedCost`) is present, so S7 programs against `cost::` **directly** (the merge prerequisite is satisfied; `book::CostInputs` is retained as the clean optimizer-facing scalar seam, not an unmerged-branch fallback). D2 — `cost::capacity_point(std::span<const risk::CapacityPoint>)` exists, so the local zero-crossing fallback is unneeded. D3 — `library::Library` exposes **no** public `store()`/`positions()` accessor, so S7-3 adds a 1-line `Library::positions(AlphaId, usize period)` read-passthrough (a second additive engine-source touch, mirroring the existing `pnl()` passthrough). D4 — there is no `admitted_ids()`/`ids_in_state()` helper, so the pipeline (§4.6) enumerates `[0, n_alphas)` filtered by `state_as_of(id, t)`. D5 — `FactorModelBuilder::build(PanelView, window, market_cap, group_id)` keys on `PanelView`; `build_components` returns `{X[0].x, F, D, fit_end = window}` (fit_begin 0). D6 — `eval::deflated_sharpe` returns `DsrResult{psr, sr_star, dsr, haircut_sharpe}` with `dsr == psr` as-built, so the decay gate's `dsr < dsr_admit` check is effectively a PSR comparison.

The spec was drafted from the research north-stars before this sprint's reconnaissance against the merged `feat/atx-core-stdlib` engine (and the then-unmerged `feat/atx-engine-cost` branch — since merged, see the v2 note above). Nine load-bearing corrections; each changes a unit's scope.
