...
| S1-5  | Bias-audit gate battery (code unit)        | done        | (this commit)  | 4     | Reusable test-support assertions in `atx::engine::validation` (`bias_audit.hpp`): `check_no_lookahead` (truncation-invariance, exact bit-for-bit compare), `check_survivorship_frozen` (delisted PnL present + frozen to 0 post-delist), `catches_overfit_synthetic` (non-vacuous: both overfit gates fire). Reuse verbatim in S3/S4/S5. Empirically verified non-vacuity: pbo=1.000000 (>0.5), single_test PSR=0.970406 (>0.9), dsr=0.086628 (<0.5). Full suite 1623/1623 green. (Sprint-close doc ceremony handled separately — code unit only.) |
...
| (this commit) | S1-5 | feat(s1-5): reusable bias-audit gate battery (lookahead/survivorship/snooping) |
