...
- `alpha-pipeline-reference.md` and `data-ingestion-reference.md` — the **two canonical reference docs** authored as **S1-1** (the user's "document the pipeline + data ingestion better, to start" ask). Listed here so future agents know they land in S1. Until then, the end-to-end maps live in the research deep-dives and per-sprint plans.
...
**Sprint merge:** ⏳ in progress (opened 2026-06-14; worktree `atx-wt/p3-s1`).
...
| 1.3 | **Total-return adjustment** — fold split (`cumulative_adjustment_factor`) + reinvested `cash_dividend` into an adjusted total-return close field; PIT-correct; validated vs known AAPL split/div events, cross-checked vs `--mode adjclose` builder output | Moderate | ✅ `S1_3_SHA` | S1-3 | `data/adjust.{hpp,cpp}` + test |
| 1.4 | **Universe / market-cap / liquidity / sector** — `shares_outstanding`→market cap; trailing-ADV liquidity screen; GICS sector field; PIT membership mask; **survivorship caveat documented** | Moderate | ⏳ pending | S1-4 | `data/universe.{hpp,cpp}` + test |
| 1.5 | **Real-data E2E path** — databento segment ⋈ corp-actions ⋈ universe → `DatasetCatalog` → `adapt_panel` → `Panel`; end-to-end test over the **actual on-disk smoke data**; **golden digest pin** on the assembled Panel | Heavy | ⏳ pending | S1-5 | `data/real_panel.{hpp,cpp}` + E2E test |
| 1.6 | **Coverage expansion** — re-run `build_us_split_adjustments.py` over the databento universe → expanded security master for a **liquid subset**; documented data-build (offline web crawl, run + recorded, not blocking engine code) | Moderate | ⏳ pending | S1-6 | data-build run + `data-ingestion-reference.md` §coverage |
| 1.7 | Close — residuals→backlog, status table, `sprint1.md`, `--no-ff` merge | Light | ⏳ pending | S1-close | — |
...
Open forks deferred to their sprints: the exact liquidity threshold + universe size (S1-4 / S2-1), the total-return reinvestment convention's tolerance vs the adjclose oracle (S1-3), and the S2 search budget (S2-1).
