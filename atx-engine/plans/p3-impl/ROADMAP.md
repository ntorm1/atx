...
**Sprint merge:** ✅ closed 2026-06-15 — `--no-ff` into `main` (worktree `atx-wt/p3-s1`, branch `feat/p3-s1-real-data`, 15 commits `a47959c..ff14687`). **8/8 units done** (S1-6 coverage crawl run post-merge 2026-06-15 → expanded master 5321 symbols/23.2M rows, gitignored data-build); suite 1657 → **1681/0/0**; user ref [`sprint1.md`](sprint1.md).
...
| 1.6 | **Coverage expansion** — re-run `build_us_split_adjustments.py` over the databento universe → expanded security master for a **liquid subset**; documented data-build (offline web crawl, run + recorded, not blocking engine code) | Moderate | ✅ done | S1-6 | crawl **run** 2026-06-15 (post-merge data-build; output gitignored `data/`, no commit). Top-500 liquid common-equity (trailing-21d $-vol ∩ Nasdaq common, `derive_liquid_universe.py`) → expanded master **5321 symbols / 23.2M rows / 1962→2026-06-12**, schema-parity ✓, AAPL spot-check ✓ (14.687B shares). Non-destructive `--incremental`; SEC unchanged (prior partitions), Yahoo refreshed the 500. Smoke C++ fixture untouched ⇒ suite still 1681/0/0. Details + resume in `data-ingestion-reference.md` §6.1 + the S1-6 ledger row |
...
### S3 — Single-File ORATS History Loader *(✅ closed — feeds S2)*
...
**Sprint merge:** ✅ closed — `--no-ff` merge of `feat/p3-s3-orats-loader` to `main`. Engine suite 2548/2548; golden real-panel digest `0x2a22a873483d9157` unchanged. User-ref [`sprint3.md`](sprint3.md).
...
| 3.6 | E2E smoke through the unchanged p2 pipeline (synthetic always-run + guarded real) | Heavy | ✅ | S3-6 + S3-final |
| 3.7 | Close — docs, `sprint3.md`, residuals→backlog, `--no-ff` merge | Light | ✅ | S3-close |

**Deferred (operator):** the full-US-universe 11 GB data-build (the headline E2E on real data) is a documented manual step — `OratsE2ESmoke.OperatorOratsZip`, see [`data-ingestion-reference.md`](data-ingestion-reference.md) §7.6. The code path is proven in CI by the synthetic always-run integration test.
