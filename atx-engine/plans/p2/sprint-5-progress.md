...
| S5-3b | Autoencoder factor extractor — `learn/autoencoder_alpha.{hpp,cpp}` + `ModelKind::Autoencoder`        | ⏳     | —          | —     | `fit_autoencoder_factors` (GKX-style, trailing-fit/OOS-encode); **linear-AE→atx-core `pca()` subspace pin (R7)** + reconstruction-error pin; Trainer L1 wiring. |
