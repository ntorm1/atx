...
| S6-4  | `adapt_panel` (Price→Panel) + `adapt_feature` (Feature merge)      | ✅ done (4a+4b) | 4f7b13f / 444bb15 | 10 | S6.4a (`4f7b13f`): `data/adapt_panel.hpp` + `.cpp` + `data_adapt_panel_test.cpp`; `Dataset::mask()` accessor; boundary-pin seed `PriceOnlyPanelEqualsWithDatafields` byte-for-byte via `memcpy`→u64. S6.4b: `data/adapt_feature.hpp` + `.cpp` + `data_adapt_feature_test.cpp` — `merge_features_into_panel(panel_in, price, feature)` aligns via `align_onto` then APPENDS aligned columns as named fields (existing fields/order untouched → price-only path unaffected); uncovered cells NaN; name-collision → Err. 5 DataAdaptFeature tests incl. `AlignedFeatureReferenceableInFeatureSpec` (merged feature feeds `learn::build_features`) and `MissingFeatureCellMakesRowInvalid` (NaN → `row_valid==0`, M8). 10/10 DataAdapt* green; 55/55 Data*. |
| S6-5  | `adapt_signal` (Signal→AlphaCandidate via extract_streams)         | ✅ done     | b5fecb5 | 5    | `data/adapt_signal.hpp` + `.cpp` + `data_adapt_signal_test.cpp`. `signal_to_candidates(signal, price, panel, sim, policy, as_of)` → `align_onto` → build `SignalSet` (one alpha per aligned column, date-major, panel dates/instruments) → `alpha::extract_streams` → one `library::AlphaCandidate` per column via `combine::compute_metrics` (**book_size 1.0** — gross-normalized-fraction / mined-path convention). Returns an OWNING `SignalAdmission{streams, candidates}` (candidate pnl/pos_flat spans point INTO the owned `AlphaStreams`, so they outlive `admit()`; mirrors `learn/ensemble.hpp`). NO new admission code — caller does `lib.admit(c, gate)`. Provenance externally-sourced: `expr_source="<external:<source>:<column>>"`, `parent_hashes={}`, `mutation_op=seed=0`. `canon_hash = hash_bytes("external"\0 source \0 column)` — deterministic, distinct per column/source, **disjoint from the genome-hash space by construction** (mined `canonical_hash` never hashes the "external" tag). Signal-as-feature path reuses S6.4b `merge_features_into_panel` verbatim. Tests: `SignalColumnAdmitsThroughGate` (strong planted foresight column Accepts; rotating "noise" column rejected — non-vacuous both ways), `AdmitVerdictMatchesExtractStreamsPath` (genuine DIFFERENTIAL: adapter candidate == hand `extract_streams`+`compute_metrics`, equal `AdmitVerdict.kind` across two fresh libraries), `ProvenanceFlagsExternalSource`, `CanonHashDistinctFromMinedGenome`, `SignalAsFeatureReferenceable`. 5/5 DataAdaptSignal green; 116/116 Data*+Library* green; `/W4 /WX` clean. |
