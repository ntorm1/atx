...
| S6-6  | `FactorModelArtifact` + `adapt_factor` (BYO factor model)          | ✅ done     | TBD    | 6     | `data/factor_model_artifact.hpp` + `data/adapt_factor.{hpp,cpp}` + `data_adapt_factor_test.cpp`. `artifact_to_factor_model` forwards to `FactorModel::create` (single validation source, byte-identical apply). `reference_spans` aligns market_cap/group_id onto price axis as-of a date; missing inst → NaN/default_group. 6/6 DataAdaptFactor green; 226/226 Data*+Risk* green. |
