...
> **Ledger state:** ✅ **CLOSED** @ `8c36e0f` (P3c-4) — sprint 3c (mass evaluation + VM→loop bridge) complete:
> 5 units, +39 alpha-DSL tests over Phase 3b's 358 (batch 10 · streams 10 · VmSignalSource 10 · integration 9),
> full Alpha+VmSignalSource+Backtest run **392/392** green. Closes the Phase-3 extension; baton → Phase 4
> (combiner/risk). Scope changes went in *Plan adjustments*, **not** the frozen plan.
...
_(Lift to ROADMAP future-work backlog at close — left for the concurrent ROADMAP owner per adj. E.)_
...
| `8c36e0f` | P3c-4 | Phase3cIntegration 9/9; full Alpha+VmSignalSource+Backtest run 392/392/0/0; `/W4 /permissive- /WX` clean; clang-format applied; bench builds + runs (extract_streams + VmSignalSource) |

---

## What Phase 3c proves / Next sprint priorities

**What 3c proves (the Phase-3→4 baton).** The alpha DSL is no longer a single-expression evaluator — it is a
**batch research surface with a verified bridge into the backtester**:

1. **Mass evaluation is the default path, and cross-alpha CSE is now measured.** `parse_program` →
   `analyze` → `compile` already folds N named alphas into ONE hash-consed DAG; `compile_batch` is the thin
   convenience over it. The CSE lever Phase-3's P3-4 table left empty is now reportable off `Program`
   (`unique_nodes`/`total_ast_nodes`/`cache_hits`/`cache_hit_pct`, invariant
   `intern_attempts == cache_hits + unique_nodes`) — **~74% of lowered nodes fold** on a mined 24-alpha battery
   (cache_hit_pct 73.7%). Batch == singly (cell-identical, NaN==NaN) and order-independent (by-name-sorted hash
   stable; raw hash order-sensitive — proven non-vacuously in the integration suite).
2. **Determinism is byte-stable and mutation-sensitive.** A mixed-family batch over a panel WITH delisted
   instruments (universe→0) + scattered NaN gaps hashes identically on replay, and flips under each of
   reorder/perturb-cell/add-late-row — the engine is a pure deterministic function of the alpha SET + panel.
3. **The VM drives the REAL `BacktestLoop`.** `VmSignalSource` (rewritten to the as-built
   `Engine(const Panel&)`/`evaluate→SignalSet`/`required_lookback` API) transposes the loop's newest-first
   `PanelView` into a chronological `alpha::Panel`, runs the VM, and feeds the current-date cross-section to the
   loop → Portfolio. Costs-off replay is byte-identical (fills + equity curve); the `Delay{Same,Next}` knob is
   genuinely opt-in (default **Next** keeps the structural no-look-ahead firewall; `Delay::Same` fills one bar
   earlier via a dedicated gated post-queue settle — no default-path weakening).
4. **Phase 4 has a typed feed.** `extract_streams → AlphaStreams` gives per-alpha PnL + position streams
   (dense `[n_alphas][n_periods]` / `[n_alphas][n_periods][n_instruments]` spans) reusing `WeightPolicy` +
   `ExecutionSimulator` — no new portfolio logic. A trivial in-test Sharpe consumer over `pnl(0)` confirms the
   handoff shape end-to-end.

**Documented residuals carried forward** (see *P3c deferred residuals*): turnover-cost is a linear-PerDollar
approximation (full FIFO/impact loop remains authoritative for a sized backtest); the analytic stream is
constant-weight (exact vs the loop only against an undrifted base); `VmSignalSource::evaluate` allocates a fresh
`alpha::Panel`/`Engine` per rebalance (no `Engine::rebind`; cold-ish research cadence, transpose/extract are
steady-state alloc-free). None block Phase 4.

**Next sprint priorities (Phase 4 — signal combination + risk + optimizer).** Consume `AlphaStreams`: a
**combiner** (IC-weighted / cross-sectional blend over the per-alpha streams), **Barra-style risk** (factor
exposures + covariance), and a **portfolio optimizer** (mean-variance / risk-parity under turnover + gross/net
constraints) producing target weights the existing `WeightPolicy`/`ExecutionSimulator`/`BacktestLoop` already
execute. Lift the three documented residuals into the ROADMAP future-work backlog. Position-based combiner,
parallel batch + Linux TSan, and computed-goto/JIT remain deferred.

**ROADMAP status flip** (Phase-3b/3c → done, `Last reviewed` bump) is left for the concurrent shared-branch
owner — `ROADMAP.md` carries uncommitted concurrent edits and is on the do-not-touch list (adj. E; same posture
as the Phase-3 + 3b closes). Baton → **Phase 4** (signal combination + Barra risk + optimizer).
