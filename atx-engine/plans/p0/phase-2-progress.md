...
| `<PENDING2>` | P2-4 winsorize | 20/20/0/0 (+WeightPolicy.ZScoreWinsorize_ClampsOutlierBeforeStandardizing; NaNEntry strengthened to 2 live names so the NaN is the only exact zero). Adds the frozen-spec `winsorize` first step (`winsorize_limit` default 0.025) — corrects the prior silent omission. |
