...
TEST(BlackScholes, GreeksMatchFiniteDifference) {
  ...
  for (const bool call : {true, false}) {
    ...
    const double h = 1e-4;
    const double dS = (bs::bs_price(S + h, K, T, r, q, sig, call) -
                       bs::bs_price(S - h, K, T, r, q, sig, call)) / (2 * h);
    const double d2S = (bs::bs_price(S + h, K, T, r, q, sig, call) -
                        2 * bs::bs_price(S, K, T, r, q, sig, call) +
                        bs::bs_price(S - h, K, T, r, q, sig, call)) / (h * h);
    const double dSig = (bs::bs_price(S, K, T, r, q, sig + h, call) -
                         bs::bs_price(S, K, T, r, q, sig - h, call)) / (2 * h);
    const double dT = (bs::bs_price(S, K, T + h, r, q, sig, call) -
                       bs::bs_price(S, K, T - h, r, q, sig, call)) / (2 * h);
    EXPECT_NEAR(g.delta, dS, 1e-4) << "call=" << call;
    EXPECT_NEAR(g.gamma, d2S, 1e-3) << "call=" << call;
    EXPECT_NEAR(g.vega, dSig, 1e-3) << "call=" << call;
    EXPECT_NEAR(g.theta, -dT / 365.0, 1e-5) << "call=" << call;
  }
}
