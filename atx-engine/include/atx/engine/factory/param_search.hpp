...
[[nodiscard]] inline ParamSpace box(const std::vector<std::pair<atx::f64, atx::f64>> &bounds) {
  ParamSpace sp;
...
[[nodiscard]] inline ParamSpace extract_free_constants(const Genome &g) {
  ParamSpace sp;
...
[[nodiscard]] inline SepCmaParams sep_cma_params(atx::usize K, atx::usize lambda) {
  SepCmaParams p;
