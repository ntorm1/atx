...
// ===========================================================================
//  pairwise_complete_cov — sample COVARIANCE over the BOTH-non-NaN overlap.
//
//  The covariance analog of pairwise_complete_corr, kept ALONGSIDE it so the
//  combination layer has ONE documented §3.3 pairwise-complete NaN policy (the
//  combiner's ShrinkageMv estimates the cross-alpha sample covariance matrix
//  S = cov(R) entry-by-entry with this helper, exactly as the gate estimates
//  correlation with pairwise_complete_corr). Same skip rule: an index where
//  EITHER leg is NaN contributes to nothing.
//
//  PRECONDITION: a.size() == b.size() (asserted in debug; paired observations).
//  Release build is OOB-safe via the min() loop bound (see pairwise_complete_corr).
//
//  SAMPLE (N-1) divisor — the unbiased estimator, matching atx-core pca()/Eigen
//  cov conventions (divide by n_valid - 1). The diagonal cov(a,a) is therefore
//  the sample VARIANCE of a over its non-NaN observations.
//
//  DEGENERATE-PAIR CONVENTION: fewer than 2 valid pairs -> 0.0 (no co-movement
//  information; matches the corr helper's < 2 rule). With exactly 0 or 1 valid
//  pairs the (n-1) divisor is undefined, so 0.0 is both the meaningful and the
//  numerically safe answer.
//
//  STRUCTURAL INDEX-0 NOTE: index 0 (the AlphaStreams structural 0) is non-NaN, so
//  per §3.3 it IS a valid pair and is INCLUDED here. The combiner's IcWeighted
//  proxy excludes index 0 from its per-alpha MOMENTS (the P4-2 §0-F convention,
//  because a structural 0 biases mean/variance) — but the covariance MATRIX uses
//  this verbatim-§3.3 helper, which keeps index 0 (a shared leading point is
//  mean-centered away and does not bias a covariance the way it biases a raw mean).
//  The caller passes the [fit_begin, fit_end) sub-span, so when fit_begin > 0 the
//  structural 0 is simply not in the window and the distinction is moot.
//
//  Thread-safety: a pure function of its inputs; no shared state.
// ===========================================================================
[[nodiscard]] inline atx::f64 pairwise_complete_cov(std::span<const atx::f64> a,
                                                    std::span<const atx::f64> b) noexcept {
  ATX_ASSERT(a.size() == b.size());
  atx::f64 sa = 0.0;  // Σ a_i over valid pairs
  atx::f64 sb = 0.0;  // Σ b_i
  atx::f64 sab = 0.0; // Σ a_i·b_i
  atx::usize n = 0U;  // count of valid (both-non-NaN) pairs
  // SAFETY: bounded by min(a.size(), b.size()) — release OOB-safe even on a
  //         contract violation (the assert fires loudly in debug). Identical
  //         rationale to pairwise_complete_corr above.
  const atx::usize bound = std::min(a.size(), b.size());
  for (atx::usize i = 0U; i < bound; ++i) {
    const atx::f64 ai = a[i];
    const atx::f64 bi = b[i];
    if (std::isnan(ai) || std::isnan(bi)) {
      continue; // pairwise-complete: skip if EITHER leg is NaN
    }
    sa += ai;
    sb += bi;
    sab += ai * bi;
  ...
  // < 2 valid pairs: undefined (n-1) divisor / no co-movement info -> 0.
  if (n < 2U) {
    return 0.0;
  }
  const atx::f64 nf = static_cast<atx::f64>(n);
  // cov = (Σab − Σa·Σb/n) / (n − 1)  (sample, unbiased).
  return (sab - sa * sb / nf) / (nf - 1.0);
}
